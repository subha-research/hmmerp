frappe.provide("hmmerp.financial_statements");

hmmerp.financial_statements = {
	filters: get_filters(),
	baseData: null,
	formatter: function (value, row, column, data, default_formatter, filter) {
		...
		if (data && column.fieldname == this.name_field) {
			...
			if (data.account || data.accounts) {
				column.link_onclick =
					"hmmerp.financial_statements.open_general_ledger(" + JSON.stringify(data) + ")";
			}
			column.is_tree = true;
		}
	...
	onload: function (report) {
		...
		hmmerp.financial_statements.filters = get_filters();

		let fiscal_year = hmmerp.utils.get_fiscal_year(frappe.datetime.get_today());
...
function get_filters() {
	let filters = [
		...
		// accordingly.
		{
			...
			label: __("Currency"),
			fieldtype: "Select",
			options: hmmerp.get_presentation_currency_list(),
		},
		{
	...
	let fiscal_year = hmmerp.utils.get_fiscal_year(frappe.datetime.get_today(), false, false);
	if (fiscal_year) {
