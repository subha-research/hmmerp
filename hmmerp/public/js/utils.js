...
frappe.provide("hmmerp");
frappe.provide("hmmerp.utils");
frappe.provide("hmmerp.stock.utils");
...
$.extend(hmmerp, {
	...
	hide_company: function (frm) {
		if (frm?.fields_dict.company) {
			...
			} else if (hmmerp.last_selected_company) {
				if (!frm.doc.company) frm.set_value("company", hmmerp.last_selected_company);
			}
		}
	},
...
$.extend(hmmerp.utils, {
		...
		if (frm.doc.__onload && frm.doc.__onload.dashboard_info) {
			...
			if (company_wise_info.length > 1) {
				company_wise_info.forEach(function (info) {
					hmmerp.utils.add_indicator_for_multicompany(frm, info);
				});
	...
	get_terms: function (tc_name, doc, callback) {
		if (tc_name) {
			return frappe.call({
				method: "hmmerp.setup.doctype.terms_and_conditions.terms_and_conditions.get_terms_and_conditions",
				args: {
	...
	make_bank_account: function (doctype, docname) {
		frappe.call({
			method: "hmmerp.accounts.doctype.bank_account.bank_account.make_bank_account",
			args: {
				doctype: doctype,
				docname: docname,
			},
	...
	add_dimensions: function (report_name, index) {
		...
		frappe.call({
			method: "hmmerp.accounts.doctype.accounting_dimension.accounting_dimension.get_dimensions",
			callback: function (r) {
	...
	add_inventory_dimensions: function (report_name, index) {
		...
		frappe.call({
			method: "hmmerp.stock.doctype.inventory_dimension.inventory_dimension.get_inventory_dimensions",
			callback: function (r) {
	...
	make_pricing_rule: function (doctype, docname) {
		frappe.call({
			method: "hmmerp.accounts.doctype.pricing_rule.pricing_rule.make_pricing_rule",
			args: {
				doctype: doctype,
				docname: docname,
			},
	...
	get_fiscal_year: function (date, with_dates = false, raise_on_missing = true) {
		...
		if (
			...
			frappe.call({
				method: "hmmerp.accounts.utils.get_fiscal_year",
...
hmmerp.utils.select_alternate_items = function (opts) {
	const frm = opts.frm;
			...
			{ fieldtype: "Section Break", label: __("Items") },
			{
				...
				fields: [
					...
					},
					{
						...
						get_query: (e) => {
							return {
								query: "hmmerp.stock.doctype.item_alternative.item_alternative.get_alternative_items",
								filters: {
...
hmmerp.utils.update_child_items = function (opts) {
	const frm = opts.frm;
	...
	let dialog = new frappe.ui.Dialog({
		...
		update_items: function () {
			...
			frappe.call({
				method: "hmmerp.controllers.accounts_controller.update_child_qty_rate",
				freeze: true,
				args: {
...
hmmerp.utils.map_current_doc = function (opts) {
	function _map() {
	...
	$.each(frappe.boot.service_level_agreement_doctypes, function (_i, d) {
		frappe.ui.form.on(d, {
			onload: function (frm) {
				...
				frappe.call({
					method: "hmmerp.support.doctype.service_level_agreement.service_level_agreement.get_service_level_agreement_filters",
					args: {
						doctype: frm.doc.doctype,
...
$.extend(hmmerp.stock.utils, {
	set_item_details_using_barcode(frm, child_row, callback) {
		const barcode_scanner = new hmmerp.utils.BarcodeScanner({ frm: frm });
