...
function render_clear_demo_action() {
	let demo_action = $(
		`<a class="dropdown-item" onclick="return hmmerp.demo.clear_demo()">
			${__("Clear Demo Data")}
		</a>`
	);
...
hmmerp.demo.clear_demo = function () {
	frappe.confirm(__("Are you sure you want to clear all demo data?"), () => {
		frappe.call({
			method: "hmmerp.setup.demo.clear_demo_data",
			freeze: true,
