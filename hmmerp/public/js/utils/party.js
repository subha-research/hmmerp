...
hmmerp.utils.get_party_details = function (frm, method, args, callback) {
	if (!method) {
	...
	if (frappe.meta.get_docfield(frm.doc.doctype, "taxes")) {
		if (
			!hmmerp.utils.validate_mandatory(
				frm,
...
hmmerp.utils.add_item = function (frm) {
	if (frm.is_new()) {
...
hmmerp.utils.get_address_display = function (frm, address_field, display_field, is_your_company_address) {
...
hmmerp.utils.set_taxes_from_address = function (
	frm,
	...
	frappe.call({
		method: "hmmerp.accounts.party.get_address_tax_category",
		args: {
		...
		callback: function (r) {
			if (!r.exc) {
				if (frm.doc.tax_category != r.message) {
					...
					hmmerp.utils.set_taxes(frm, triggered_from_field);
				}
			}
		},
	});
...
hmmerp.utils.set_taxes = function (frm, triggered_from_field) {
	if (frappe.meta.get_docfield(frm.doc.doctype, "taxes")) {
		if (!hmmerp.utils.validate_mandatory(frm, "Company", frm.doc.company, triggered_from_field)) {
			return;
		}
	...
	frappe.call({
		method: "hmmerp.accounts.party.set_taxes",
		args: {
			party: party,
			party_type: party_type,
...
hmmerp.utils.get_contact_details = function (frm) {
	if (frm.updating_party_details) return;
...
hmmerp.utils.validate_mandatory = function (frm, label, value, trigger_on) {
	if (!value) {
		frm.doc[trigger_on] = "";
...
hmmerp.utils.get_shipping_address = function (frm, callback) {
	if (frm.doc.company) {
		if (
		...
		frappe.call({
			method: "hmmerp.accounts.custom.address.get_shipping_address",
			args: {
				company: frm.doc.company,
