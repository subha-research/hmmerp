...
hmmerp.accounts.unreconcile_payment = {
	add_unreconcile_btn(frm) {
		if (frm.doc.docstatus == 1) {
			...
			frappe.call({
				method: "hmmerp.accounts.doctype.unreconcile_payment.unreconcile_payment.doc_has_references",
				args: {
				...
				callback: function (r) {
					if (r.message) {
						frm.add_custom_button(
							__("UnReconcile"),
							function () {
								hmmerp.accounts.unreconcile_payment.build_unreconcile_dialog(frm);
							},
							__("Actions")
						);
					}
	...
	build_unreconcile_dialog(frm) {
		if (
			...
			// get linked payments
			frappe.call({
				method: "hmmerp.accounts.doctype.unreconcile_payment.unreconcile_payment.get_linked_payments_for_doc",
				args: {
						...
						let d = new frappe.ui.Dialog({
							...
							primary_action(values) {
								...
								if (selected_allocations.length > 0) {
									let selection_map =
										hmmerp.accounts.unreconcile_payment.build_selection_map(
											frm,
											selected_allocations
										);
									hmmerp.accounts.unreconcile_payment.create_unreconcile_docs(
										selection_map
									);
									d.hide();
								} else {
	...
	create_unreconcile_docs(selection_map) {
		frappe.call({
			method: "hmmerp.accounts.doctype.unreconcile_payment.unreconcile_payment.create_unreconcile_doc_for_selection",
			args: {
