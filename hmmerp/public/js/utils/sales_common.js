...
frappe.provide("hmmerp.selling");

hmmerp.sales_common = {
	setup_selling_controller: function () {
		hmmerp.selling.SellingController = class SellingController extends hmmerp.TransactionController {
			setup() {
				super.setup();
			...
			onload() {
				...
				this.frm.set_query("project", function (doc) {
					return {
						query: "hmmerp.controllers.queries.get_project_name",
						filters: {
			...
			setup_queries() {
				...
				$.each(
					...
					function (i, opts) {
						if (me.frm.fields_dict[opts[0]]) me.frm.set_query(opts[0], hmmerp.queries[opts[1]]);
					}
				);

				me.frm.set_query("contact_person", hmmerp.queries.contact_query);
				me.frm.set_query("company_contact_person", hmmerp.queries.company_contact_query);
				me.frm.set_query("customer_address", hmmerp.queries.address_query);
				me.frm.set_query("shipping_address_name", hmmerp.queries.address_query);
				me.frm.set_query("dispatch_address_name", hmmerp.queries.dispatch_address_query);
				me.frm.set_query("company_address", hmmerp.queries.company_address_query);
			...
			set_default_company_address() {
				...
				if (this.frm.doc.company) {
					frappe.call({
						method: "hmmerp.setup.doctype.company.company.get_default_company_address",
						args: {
			...
			customer_address() {
				hmmerp.utils.get_address_display(this.frm, "customer_address");
				hmmerp.utils.set_taxes_from_address(
					this.frm,
					"customer_address",
					"customer_address",
			...
			shipping_address_name() {
				hmmerp.utils.get_address_display(this.frm, "shipping_address_name", "shipping_address");
				hmmerp.utils.set_taxes_from_address(
					this.frm,
					"shipping_address_name",
					"customer_address",
			...
			dispatch_address_name() {
				hmmerp.utils.get_address_display(this.frm, "dispatch_address_name", "dispatch_address");
			}
			...
			pick_serial_and_batch(doc, cdt, cdn) {
				...
				frappe.db.get_value("Item", item.item_code, ["has_batch_no", "has_serial_no"]).then((r) => {
					if (r.message && (r.message.has_batch_no || r.message.has_serial_no)) {
						...
						new hmmerp.SerialBatchPackageSelector(me.frm, item, (r) => {
							if (r) {
			...
			project() {
				...
				if (["Delivery Note", "Sales Invoice", "Sales Order"].includes(this.frm.doc.doctype)) {
					if (this.frm.doc.project) {
						frappe.call({
							method: "hmmerp.projects.doctype.project.project.get_cost_center_name",
