class BOMConfigurator {
	...
	tree_options() {
		return {
			...
			get_tree_nodes: "hmmerp.manufacturing.doctype.bom_creator.bom_creator.get_children",
	...
	add_item(node, view) {
		frappe.prompt(
			...
			(data) => {
				...
				frappe.call({
					method: "hmmerp.manufacturing.doctype.bom_creator.bom_creator.add_item",
					args: {
	...
	add_sub_assembly(node, view) {
		...
		dialog.set_primary_action(__("Add"), () => {
			...
			frappe.call({
				method: "hmmerp.manufacturing.doctype.bom_creator.bom_creator.add_sub_assembly",
				args: {
					parent: node.data.parent_id,
					fg_item: node.data.value,
					fg_reference_id: node.data.name || this.frm.doc.name,
				...
				...[
					{ fieldtype: "Section Break" },
					{
						...
						get_query() {
							...
							if (doc.routing) {
								return {
									query: "hmmerp.manufacturing.doctype.routing.routing.get_operations",
									filters: {
										routing: doc.routing,
									},
								};
	...
	delete_node(node, view) {
		frappe.confirm(__("Are you sure you want to delete this Item?"), () => {
			frappe.call({
				method: "hmmerp.manufacturing.doctype.bom_creator.bom_creator.delete_node",
				args: {
					parent: node.data.parent_id,
					fg_item: node.data.value,
	...
	edit_bom(node, view) {
		...
		this.frm.edit_bom_dialog = frappe.prompt(
			...
			(data) => {
				...
				frappe.call({
					method: "hmmerp.manufacturing.doctype.bom_creator.bom_creator.edit_bom_creator",
					args: {
						doctype: doctype,
						docname: docname,
						data: data,
