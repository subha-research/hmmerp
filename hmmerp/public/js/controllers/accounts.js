...
// get tax rate
frappe.provide("hmmerp.taxes");

hmmerp.accounts.taxes = {
			...
			onload: function(frm) {
				if(frm.get_field("taxes")) {
					frm.set_query("account_head", "taxes", function(doc) {
						...
						return {
							query: "hmmerp.controllers.queries.tax_account_query",
							filters: {
		...
		frappe.ui.form.on(doctype, {
			account_head: function(frm, cdt, cdn) {
				...
				if(!d.charge_type && d.account_head){
					...
					frappe.call({
						type:"GET",
						method: "hmmerp.controllers.accounts_controller.get_tax_rate",
...
hmmerp.accounts.payment_triggers = {
...
hmmerp.accounts.pos = {
	...
	get_payment_mode_account: function(frm, mode_of_payment, callback) {
		...
		return  frappe.call({
			method: "hmmerp.accounts.doctype.sales_invoice.sales_invoice.get_bank_cash_account",
			args: {
