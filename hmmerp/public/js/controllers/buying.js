...
hmmerp.buying = {
	setup_buying_controller: function() {
		hmmerp.buying.BuyingController = class BuyingController extends hmmerp.TransactionController {
			setup() {
				super.setup();
				...
				if(this.frm.get_field('shipping_address')) {
					this.frm.set_query("shipping_address", () => {
						if(this.frm.doc.customer) {
							...
							return hmmerp.queries.company_address_query(this.frm.doc)
					});
				}

				if(this.frm.get_field('dispatch_address')) {
					this.frm.set_query("dispatch_address", () => {
						return hmmerp.queries.address_query(this.frm.doc);
					});
				}
			}
			...
			setup_queries(doc, cdt, cdn) {
				...
				me.frm.set_query('supplier', hmmerp.queries.supplier);
				me.frm.set_query('contact_person', hmmerp.queries.contact_query);
				me.frm.set_query('supplier_address', hmmerp.queries.address_query);

				me.frm.set_query('billing_address', hmmerp.queries.company_address_query);
				hmmerp.accounts.dimensions.setup_dimension_filters(me.frm, me.frm.doctype);
				...
				this.frm.set_query("manufacturer", "items", function(doc, cdt, cdn) {
					...
					return {
						query: "hmmerp.controllers.queries.item_manufacturer_query",
			...
			supplier() {
				var me = this;
				hmmerp.utils.get_party_details(this.frm, null, null, function(){
					me.apply_price_list();
				});
			}
			...
			company(){
				...
				frappe.call({
					method: "hmmerp.setup.doctype.company.company.get_billing_shipping_address",
					args: {
				...
				hmmerp.utils.set_letter_head(this.frm)
			}

			supplier_address() {
				hmmerp.utils.get_address_display(this.frm);
				hmmerp.utils.set_taxes_from_address(this.frm, "supplier_address", "supplier_address", "supplier_address");
			}
			...
			warehouse(doc, cdt, cdn) {
				...
				if(item.item_code && item.warehouse) {
					return this.frm.call({
						method: "hmmerp.stock.get_item_details.get_bin_details",
						child: item,
						args: {
			...
			set_from_product_bundle() {
				...
				this.frm.add_custom_button(__("Product Bundle"), function() {
					hmmerp.buying.get_items_from_product_bundle(me.frm);
				}, __("Get Items From"));
			}

			shipping_address(){
				var me = this;
				hmmerp.utils.get_address_display(this.frm, "shipping_address",
					"shipping_address_display", true);
			}

			dispatch_address(){
				var me = this;
				hmmerp.utils.get_address_display(this.frm, "dispatch_address",
					"dispatch_address_display", true);
			}

			billing_address() {
				hmmerp.utils.get_address_display(this.frm, "billing_address",
			...
			manufacturer(doc, cdt, cdn) {
				...
				if(row.manufacturer) {
					frappe.call({
						method: "hmmerp.stock.doctype.item_manufacturer.item_manufacturer.get_item_manufacturer_part_no",
						args: {
				...
				frappe.db.get_value("Item", item.item_code, fields)
					.then((r) => {
						if (r.message && (r.message.has_batch_no || r.message.has_serial_no)) {
							...
							new hmmerp.SerialBatchPackageSelector(
								me.frm, item, (r) => {
									if (r) {
...
hmmerp.buying.link_to_mrs = function(frm) {
	frappe.call({
...
hmmerp.buying.get_default_bom = function(frm) {
	$.each(frm.doc["items"] || [], function(i, d) {
		if (d.item_code && d.bom === "") {
			return frappe.call({
				type: "GET",
				method: "hmmerp.stock.get_item_details.get_default_bom",
				args: {
...
hmmerp.buying.get_items_from_product_bundle = function(frm) {
	var dialog = new frappe.ui.Dialog({
		...
		primary_action(args){
			...
			return frappe.call({
				type: "GET",
				method: "hmmerp.stock.doctype.packed_item.packed_item.get_items_from_product_bundle",
				args: {
					row: {
