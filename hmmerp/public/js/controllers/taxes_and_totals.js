...
hmmerp.taxes_and_totals = class TaxesAndTotals extends hmmerp.payments {
	setup() {
	...
	initialize_taxes() {
		...
		$.each(this.frm.doc["taxes"] || [], function(i, tax) {
			...
			if (!this.discount_amount_applied) {
				hmmerp.accounts.taxes.validate_taxes_and_charges(tax.doctype, tax.name);
				hmmerp.accounts.taxes.validate_inclusive_tax(tax, this.frm);
			}
	...
	fetch_round_off_accounts() {
		...
		if (me.frm.doc.company) {
			frappe.call({
				"method": "hmmerp.controllers.taxes_and_totals.get_round_off_applicable_accounts",
				"args": {
		...
		frappe.call({
			method: "hmmerp.controllers.taxes_and_totals.get_rounding_tax_settings",
			callback: function(r) {
		...
		*/
		...
		if(this.frm.doc.return_against){
			let {message : return_against_mop } = await frappe.call({
				method: 'hmmerp.controllers.sales_and_purchase_return.get_payment_data',
				args: {
