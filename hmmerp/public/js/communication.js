frappe.ui.form.on("Communication", {
	...
	make_lead_from_communication: (frm) => {
		return frappe.call({
			method: "hmmerp.crm.doctype.lead.lead.make_lead_from_communication",
			args: {
	...
	make_issue_from_communication: (frm) => {
		return frappe.call({
			method: "hmmerp.support.doctype.issue.issue.make_issue_from_communication",
			args: {
				communication: frm.doc.name,
			},
			freeze: true,
	...
	make_opportunity_from_communication: (frm) => {
		...
		frappe.prompt(
			...
			(data) => {
				frappe.call({
					method: "hmmerp.crm.doctype.opportunity.opportunity.make_opportunity_from_communication",
					args: {
						communication: frm.doc.name,
