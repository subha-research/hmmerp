frappe.provide("hmmerp.bulk_transaction_processing");

$.extend(hmmerp.bulk_transaction_processing, {
	create: function (listview, from_doctype, to_doctype, args) {
		...
		frappe.confirm(__("Create {0} {1} ?", [count_of_rows, __(to_doctype)]), () => {
			if (doc_name.length == 0) {
				frappe
					.call({
						method: "hmmerp.utilities.bulk_transaction.transaction_processing",
						args: {
							data: checked_items,
