...
frappe.setup.on("before_load", function () {
	if (
		...
		frappe.boot.setup_wizard_completed_apps.includes("hmmerp")
	) {
		return;
	}

	hmmerp.setup.slides_settings.map(frappe.setup.add_slide);
});

hmmerp.setup.slides_settings = [
	{
		// Organization
		name: "organization",
		...
		set_fy_dates: function (slide) {
			...
			if (country) {
				let fy = hmmerp.setup.fiscal_years[country];
		...
		charts_modal: function (slide, chart_template) {
			...
			let coa_tree = new frappe.ui.Tree({
				...
				label: parent,
				expandable: true,
				method: "hmmerp.accounts.utils.get_coa",
				args: {
