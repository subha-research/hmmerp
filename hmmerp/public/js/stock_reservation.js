frappe.provide("hmmerp.stock_reservation");

$.extend(hmmerp.stock_reservation, {
	make_entries(frm, table_name) {
		hmmerp.stock_reservation.setup(frm, table_name);
	},

	setup(frm, table_name) {
		let parms = hmmerp.stock_reservation.get_parms(frm, table_name);

		hmmerp.stock_reservation.dialog = new frappe.ui.Dialog({
			title: __("Stock Reservation"),
			size: "extra-large",
			fields: hmmerp.stock_reservation.get_dialog_fields(frm, parms),
			primary_action_label: __("Reserve Stock"),
			primary_action: () => {
				hmmerp.stock_reservation.reserve_stock(frm, table_name, parms);
			},
		});

		hmmerp.stock_reservation.render_items(frm, parms);
	},
	...
	get_parms(frm, table_name) {
		...
		params["method"] = {
			"Sales Order": "delivered_qty",
			"Production Plan":
				"hmmerp.manufacturing.doctype.production_plan.production_plan.make_stock_reservation_entries",
			"Work Order":
				"hmmerp.manufacturing.doctype.work_order.work_order.make_stock_reservation_entries",
		}[frm.doc.doctype];
	...
	get_dialog_fields(frm, parms) {
		let fields = hmmerp.stock_reservation.fields || [];
		let qty_field = parms.qty_field;
		let dialog = hmmerp.stock_reservation.dialog;
	...
	unreserve_stock(frm) {
		hmmerp.stock_reservation.get_stock_reservation_entries(frm.doctype, frm.docname).then((r) => {
			if (!r.exc && r.message) {
				if (r.message.length > 0) {
					hmmerp.stock_reservation.prepare_for_cancel_sre_entries(frm, r.message);
				} else {
			...
			fields: [
				{
					...
					in_place_edit: true,
					data: [],
					fields: hmmerp.stock_reservation.get_fields_for_cancel(),
				},
			],
			primary_action_label: __("Unreserve Stock"),
			primary_action: () => {
				hmmerp.stock_reservation.cancel_stock_reservation(dialog, frm);
			},
		});
	...
	cancel_stock_reservation(dialog, frm) {
		...
		let method = "hmmerp.manufacturing.doctype.work_order.work_order.cancel_stock_reservation_entries";

		if (frm.doc.doctype === "Production Plan") {
			method =
				"hmmerp.manufacturing.doctype.production_plan.production_plan.cancel_stock_reservation_entries";
		}
