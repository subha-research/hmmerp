...
frappe.provide("hmmerp.queries");
$.extend(hmmerp.queries, {
	user: function () {
	...
	item: function (filters) {
		var args = { query: "hmmerp.controllers.queries.item_query" };
		if (filters) args["filters"] = filters;
		return args;
	},

	bom: function () {
		return { query: "hmmerp.controllers.queries.bom" };
	},

	task: function () {
		return { query: "hmmerp.projects.utils.query_task" };
	},
	...
	employee: function () {
		return { query: "hmmerp.controllers.queries.employee_query" };
	},
	...
	get_filtered_dimensions: function (doc, child_fields, dimension, company) {
		...
		return {
			query: "hmmerp.controllers.queries.get_filtered_dimensions",
			filters: {
				dimension: dimension,
				account: account,
				company: company,
...
hmmerp.queries.setup_queries = function (frm, options, query_fn) {
	var me = this;
...
*/
hmmerp.queries.setup_warehouse_query = function (frm) {
	frm.set_query("warehouse", "items", function (doc, cdt, cdn) {
		var row = locals[cdt][cdn];
		var filters = hmmerp.queries.warehouse(frm.doc);
		if (row.item_code) {
			$.extend(filters, { query: "hmmerp.controllers.queries.warehouse_query" });
