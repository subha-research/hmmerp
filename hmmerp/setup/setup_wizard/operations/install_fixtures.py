...
from hmmerp.accounts.doctype.account.account import RootNotEditable
from hmmerp.regional.address_template.setup import set_up_address_templates
...
def install(country=None):
	...
	base_path = frappe.get_app_path("hmmerp", "stock", "doctype")
	...
	from hmmerp.buying.doctype.supplier_scorecard.supplier_scorecard import make_default_records
...
def add_uom_data():
	# add UOMs
	uoms = json.loads(
		open(frappe.get_app_path("hmmerp", "setup", "setup_wizard", "data", "uom_data.json")).read()
	)
	for d in uoms:
	...
	uom_conversions = json.loads(
		open(
			frappe.get_app_path("hmmerp", "setup", "setup_wizard", "data", "uom_conversion_data.json")
		).read()
	)
