...
frappe.provide("hmmerp.crm");
hmmerp.pre_sales.set_as_lost("Opportunity");
...
frappe.ui.form.on("Opportunity", {
	...
	party_name: function (frm) {
		...
		if (frm.doc.opportunity_from == "Customer") {
			...
			hmmerp.utils.map_current_doc({
				method: "hmmerp.crm.doctype.lead.lead.make_opportunity",
	...
	customer_address: function (frm, cdt, cdn) {
		hmmerp.utils.get_address_display(frm, "customer_address", "address_display", false);
	},

	contact_person: hmmerp.utils.get_contact_details,
	...
	make_supplier_quotation: function (frm) {
		frappe.model.open_mapped_doc({
			method: "hmmerp.crm.doctype.opportunity.opportunity.make_supplier_quotation",
			frm: frm,
		});
	},
	...
	make_request_for_quotation: function (frm) {
		frappe.model.open_mapped_doc({
			method: "hmmerp.crm.doctype.opportunity.opportunity.make_request_for_quotation",
			frm: frm,
		});
	},
...
hmmerp.crm.Opportunity = class Opportunity extends frappe.ui.form.Controller {
	onload() {
	...
	setup_queries() {
		...
		if (me.frm.doc.opportunity_from == "Lead") {
			me.frm.set_query("party_name", hmmerp.queries["lead"]);
		} else if (me.frm.doc.opportunity_from == "Customer") {
			me.frm.set_query("party_name", hmmerp.queries["customer"]);
	...
	create_quotation() {
		frappe.model.open_mapped_doc({
			method: "hmmerp.crm.doctype.opportunity.opportunity.make_quotation",
			frm: this.frm,
		});
	}
	...
	make_customer() {
		frappe.model.open_mapped_doc({
			method: "hmmerp.crm.doctype.opportunity.opportunity.make_customer",
			frm: this.frm,
		});
	}

	show_notes() {
		const crm_notes = new hmmerp.utils.CRMNotes({
			frm: this.frm,
	...
	show_activities() {
		const crm_activities = new hmmerp.utils.CRMActivities({
			frm: this.frm,
...
extend_cscript(cur_frm.cscript, new hmmerp.crm.Opportunity({ frm: cur_frm }));

cur_frm.cscript.item_code = function (doc, cdt, cdn) {
	var d = locals[cdt][cdn];
	if (d.item_code) {
		return frappe.call({
			method: "hmmerp.crm.doctype.opportunity.opportunity.get_item_details",
