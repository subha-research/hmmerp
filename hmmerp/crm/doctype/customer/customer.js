...
frappe.ui.form.on("Customer", {
	setup: function (frm) {
		...
		frm.make_methods = {
			Quotation: () =>
				frappe.model.open_mapped_doc({
					method: "hmmerp.selling.doctype.customer.customer.make_quotation",
					frm: frm,
				}),
			"Sales Order": () =>
			...
			Opportunity: () =>
				frappe.model.open_mapped_doc({
					method: "hmmerp.selling.doctype.customer.customer.make_opportunity",
					frm: frm,
				}),
			"Payment Entry": () =>
				frappe.model.open_mapped_doc({
					method: "hmmerp.selling.doctype.customer.customer.make_payment_entry",
					frm: frm,
				}),
			"Pricing Rule": () => hmmerp.utils.make_pricing_rule(frm.doc.doctype, frm.doc.name),
		};
		...
		frm.set_query("customer_primary_contact", function (doc) {
			return {
				query: "hmmerp.selling.doctype.customer.customer.get_customer_primary_contact",
				filters: {
					customer: doc.name,
				},
			};
	...
	refresh: function (frm) {
		...
		if (!frm.doc.__islocal) {
			...
			// indicator
			hmmerp.utils.set_party_dashboard_indicators(frm);
		} else {
	...
	show_party_link_dialog: function (frm) {
		const dialog = new frappe.ui.Dialog({
			...
			primary_action: function ({ supplier }) {
				frappe.call({
					method: "hmmerp.accounts.doctype.party_link.party_link.create_party_link",
					args: {
