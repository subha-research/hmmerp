...
from hmmerp.accounts.party import get_dashboard_info, validate_party_accounts
from hmmerp.controllers.website_list_for_contact import add_role_for_portal_user
...
class Customer(TransactionBase):
	...
	if TYPE_CHECKING:
		...
		from hmmerp.accounts.doctype.allowed_to_transact_with.allowed_to_transact_with import (
			AllowedToTransactWith,
		)
		from hmmerp.accounts.doctype.party_account.party_account import PartyAccount
		from hmmerp.selling.doctype.customer_credit_limit.customer_credit_limit import CustomerCreditLimit
		from hmmerp.selling.doctype.sales_team.sales_team import SalesTeam
		from hmmerp.selling.doctype.supplier_number_at_customer.supplier_number_at_customer import (
			SupplierNumberAtCustomer,
		)
		from hmmerp.utilities.doctype.portal_user.portal_user import PortalUser
	...
	def copy_communication(self):
		...
		from hmmerp.crm.utils import copy_comments, link_communications
	...
	if credit_limit > 0 and flt(customer_outstanding) > credit_limit:
		...
		if not credit_controller_role or credit_controller_role not in frappe.get_roles():
			...
			frappe.msgprint(
				...
				primary_action={
					"label": "Send Email",
					"server_action": "hmmerp.selling.doctype.customer.customer.send_emails",
