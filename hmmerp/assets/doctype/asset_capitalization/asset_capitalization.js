...
frappe.provide("hmmerp.assets");

hmmerp.assets.AssetCapitalization = class AssetCapitalization extends hmmerp.stock.StockController {
	setup() {
	...
	setup_queries() {
		...
		me.frm.set_query("target_item_code", function () {
			return hmmerp.queries.item({ is_stock_item: 0, is_fixed_asset: 1 });
		});
		...
		me.frm.set_query("item_code", "service_items", function () {
			return hmmerp.queries.item({ is_stock_item: 0, is_fixed_asset: 0 });
		});
	...
	set_consumed_stock_items_tagged_to_wip_composite_asset(target_asset) {
		...
		if (target_asset) {
			return me.frm.call({
				method: "hmmerp.assets.doctype.asset_capitalization.asset_capitalization.get_items_tagged_to_wip_composite_asset",
				args: {
					params: {
	...
	stock_items_add(doc, cdt, cdn) {
		hmmerp.accounts.dimensions.copy_dimension_from_first_row(this.frm, cdt, cdn, "stock_items");
	}

	asset_items_add(doc, cdt, cdn) {
		hmmerp.accounts.dimensions.copy_dimension_from_first_row(this.frm, cdt, cdn, "asset_items");
	}

	serivce_items_add(doc, cdt, cdn) {
		hmmerp.accounts.dimensions.copy_dimension_from_first_row(this.frm, cdt, cdn, "service_items");
	}
	...
	get_target_item_details() {
		...
		if (me.frm.doc.target_item_code) {
			return me.frm.call({
				method: "hmmerp.assets.doctype.asset_capitalization.asset_capitalization.get_target_item_details",
				args: {
	...
	get_target_asset_details() {
		...
		if (me.frm.doc.target_asset) {
			return me.frm.call({
				method: "hmmerp.assets.doctype.asset_capitalization.asset_capitalization.get_target_asset_details",
				args: {
	...
	get_consumed_stock_item_details(row) {
		...
		if (row && row.item_code) {
			return me.frm.call({
				method: "hmmerp.assets.doctype.asset_capitalization.asset_capitalization.get_consumed_stock_item_details",
				child: row,
				args: {
					ctx: {
						item_code: row.item_code,
	...
	get_consumed_asset_details(row) {
		...
		if (row && row.asset) {
			return me.frm.call({
				method: "hmmerp.assets.doctype.asset_capitalization.asset_capitalization.get_consumed_asset_details",
				child: row,
				args: {
					ctx: {
						asset: row.asset,
	...
	get_service_item_details(row) {
		...
		if (row && row.item_code) {
			return me.frm.call({
				method: "hmmerp.assets.doctype.asset_capitalization.asset_capitalization.get_service_item_details",
				child: row,
				args: {
					ctx: {
						item_code: row.item_code,
	...
	get_warehouse_details(item) {
		...
		if (item.item_code && item.warehouse) {
			me.frm.call({
				method: "hmmerp.assets.doctype.asset_capitalization.asset_capitalization.get_warehouse_details",
				child: item,
				args: {
					args: {
						item_code: item.item_code,
...
cur_frm.cscript = new hmmerp.assets.AssetCapitalization({ frm: cur_frm });
