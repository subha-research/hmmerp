...
frappe.ui.form.on("Asset", {
	...
	setup: function (frm) {
		...
		frm.make_methods = {
			"Asset Movement": () => {
				frappe.call({
					method: "hmmerp.assets.doctype.asset.asset.make_asset_movement",
					freeze: true,
					args: {
		...
		frm.set_query("purchase_receipt", (doc) => {
			return {
				query: "hmmerp.controllers.queries.get_purchase_receipts",
				filters: { item_code: doc.item_code },
			};
		});
		frm.set_query("purchase_invoice", (doc) => {
			return {
				query: "hmmerp.controllers.queries.get_purchase_invoices",
				filters: { item_code: doc.item_code },
			};
		});
	...
	refresh: function (frm) {
		...
		if (frm.doc.docstatus == 1) {
			if (["Submitted", "Partially Depreciated", "Fully Depreciated"].includes(frm.doc.status)) {
				frm.add_custom_button(
					__("Transfer Asset"),
					function () {
						hmmerp.asset.transfer_asset(frm);
					},
					__("Manage")
				);
				...
				frm.add_custom_button(__("Restore Asset"), function () {
					hmmerp.asset.restore_asset(frm);
		...
		if (frm.doc.docstatus == 0) {
			...
			if (frm.doc.is_composite_asset) {
				frappe.call({
					method: "hmmerp.assets.doctype.asset.asset.has_active_capitalization",
					args: {
						asset: frm.doc.name,
					},
					callback: function (r) {
	...
	make_journal_entry: function (frm) {
		frappe.call({
			method: "hmmerp.assets.doctype.asset.asset.make_journal_entry",
			args: {
	...
	setup_chart_and_depr_schedule_view: async function (frm) {
		...
		if (frm.doc.calculate_depreciation) {
			...
			let asset_depr_schedule_doc = (
				await frappe.call(
					"hmmerp.assets.doctype.asset_depreciation_schedule.asset_depreciation_schedule.get_asset_depr_schedule_doc",
					{
						asset_name: frm.doc.name,
						status: "Active",
	...
	set_finance_book: function (frm) {
		frappe.call({
			method: "hmmerp.assets.doctype.asset.asset.get_item_details",
			args: {
	...
	make_sales_invoice: function (frm) {
		frappe.call({
			...
			method: "hmmerp.assets.doctype.asset.asset.make_sales_invoice",
			callback: function (r) {
				var doclist = frappe.model.sync(r.message);
				frappe.set_route("Form", doclist[0].doctype, doclist[0].name);
			},
	...
	create_asset_maintenance: function (frm) {
		frappe.call({
			...
			method: "hmmerp.assets.doctype.asset.asset.create_asset_maintenance",
			callback: function (r) {
				var doclist = frappe.model.sync(r.message);
				frappe.set_route("Form", doclist[0].doctype, doclist[0].name);
			},
	...
	create_asset_repair: function (frm) {
		frappe.call({
			...
			method: "hmmerp.assets.doctype.asset.asset.create_asset_repair",
			callback: function (r) {
				var doclist = frappe.model.sync(r.message);
				frappe.set_route("Form", doclist[0].doctype, doclist[0].name);
			},
	...
	create_asset_capitalization: function (frm) {
		frappe.call({
			...
			method: "hmmerp.assets.doctype.asset.asset.create_asset_capitalization",
			callback: function (r) {
				var doclist = frappe.model.sync(r.message);
				frappe.set_route("Form", doclist[0].doctype, doclist[0].name);
	...
	split_asset: function (frm) {
		...
		dialog.set_primary_action(__("Split"), function () {
			...
			frappe.call({
				...
				method: "hmmerp.assets.doctype.asset.asset.split_asset",
				callback: function (r) {
	...
	create_asset_value_adjustment: function (frm) {
		frappe.call({
			...
			method: "hmmerp.assets.doctype.asset.asset.create_asset_value_adjustment",
			freeze: 1,
			callback: function (r) {
				var doclist = frappe.model.sync(r.message);
				frappe.set_route("Form", doclist[0].doctype, doclist[0].name);
	...
	set_values_from_purchase_doc: (frm, doctype) => {
		frappe.call({
			method: "hmmerp.assets.doctype.asset.asset.get_values_from_purchase_doc",
			args: {
...
hmmerp.asset.scrap_asset = function (frm) {
	var scrap_dialog = new frappe.ui.Dialog({
		...
		primary_action(values) {
			frappe.call({
				...
				method: "hmmerp.assets.doctype.asset.depreciation.scrap_asset",
				callback: function (r) {
					frm.reload_doc();
					scrap_dialog.hide();
				},
...
hmmerp.asset.restore_asset = function (frm) {
	frappe.confirm(__("Do you really want to restore this scrapped asset?"), function () {
		frappe.call({
			args: {
				asset_name: frm.doc.name,
			},
			method: "hmmerp.assets.doctype.asset.depreciation.restore_asset",
			callback: (r) => frm.reload_doc(),
		});
	});
};

hmmerp.asset.transfer_asset = function (frm) {
	frappe.call({
		method: "hmmerp.assets.doctype.asset.asset.make_asset_movement",
		freeze: true,
		args: {
