...
from hmmerp.accounts.party import (
	get_party_account,
...
from hmmerp.buying.utils import update_last_purchase_rate
from hmmerp.controllers.print_settings import (
	set_print_templates_for_item_table,
	set_print_templates_for_taxes,
)
from hmmerp.controllers.sales_and_purchase_return import validate_return
from hmmerp.exceptions import InvalidCurrency
from hmmerp.setup.utils import get_exchange_rate
from hmmerp.stock.doctype.item.item import get_uom_conv_factor
...
from hmmerp.utilities.regional import temporary_flag
...
class AccountsController(TransactionBase):
	...
	def company_currency(self):
		if not hasattr(self, "__company_currency"):
			self.__company_currency = hmmerp.get_company_currency(self.company)
	...
	def on_trash(self):
		from hmmerp.accounts.utils import delete_exchange_gain_loss_journal
	...
	def calculate_taxes_and_totals(self):
		from hmmerp.controllers.taxes_and_totals import calculate_taxes_and_totals
	...
	def validate_due_date(self):
		...
		from hmmerp.accounts.party import validate_due_date
			...
			for item in self.get("items"):
				if item.get("item_code"):
					...
					if hasattr(item, "cost_center") and not item.get("cost_center"):
						item.set(
							"cost_center",
							self.get("cost_center") or hmmerp.get_default_cost_center(self.company),
						)
	...
	def update_against_document_in_jv(self):
		...
		if lst:
			from hmmerp.accounts.utils import reconcile_against_document
	...
	def on_cancel(self):
		from hmmerp.accounts.doctype.bank_transaction.bank_transaction import (
				...
				self.get_gl_dict(
					{
						...
						"cost_center": self.cost_center or hmmerp.get_default_cost_center(self.company),
					},
					item=self,
				)
			)
	...
	def validate_multiple_billing(self, ref_dt, item_ref_dn, based_on):
		from hmmerp.controllers.status_updater import get_allowance_for
	...
	def get_company_default(self, fieldname, ignore_validation=False):
		from hmmerp.accounts.utils import get_company_default
	...
	def create_advance_and_reconcile(self, party_link):
		...
		default_currency = hmmerp.get_company_currency(self.company)
		...
		reconcilation_entry.cost_center = self.cost_center or hmmerp.get_default_cost_center(self.company)

		advance_entry.account = primary_account
		advance_entry.party_type = primary_party_type
		advance_entry.party = primary_party
		advance_entry.cost_center = self.cost_center or hmmerp.get_default_cost_center(self.company)
...
def update_bin_on_delete(row, doctype):
	...
	from hmmerp.stock.stock_balance import (
		get_indented_qty,
		get_ordered_qty,
		get_reserved_qty,
		update_bin_qty,
...
def update_child_qty_rate(parent_doctype, trans_items, parent_doctype_name, child_docname="items"):
	...
	if parent_doctype == "Sales Order":
		from hmmerp.stock.doctype.stock_reservation_entry.stock_reservation_entry import (
...
def merge_taxes(source_taxes, target_doc):
	from hmmerp.accounts.doctype.pos_invoice_merge_log.pos_invoice_merge_log import (
