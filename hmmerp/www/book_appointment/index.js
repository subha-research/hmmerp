...
async function get_global_variables() {
	...
	window.appointment_settings = (
		await frappe.call({
			method: "hmmerp.www.book_appointment.index.get_appointment_settings",
		})
	).message;
	window.timezones = (
		await frappe.call({
			method: "hmmerp.www.book_appointment.index.get_timezones",
		})
	).message;
}
...
async function get_time_slots(date, timezone) {
	let slots = (
		await frappe.call({
			method: "hmmerp.www.book_appointment.index.get_appointment_slots",
			args: {
				date: date,
				timezone: timezone,
			},
...
async function submit() {
	...
	let appointment = frappe.call({
		method: "hmmerp.www.book_appointment.index.create_appointment",
		args: {
