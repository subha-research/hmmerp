...
frappe.ui.form.on("Project", {
	...
	onload: function (frm) {
		...
		frm.set_query("user", "users", function () {
			return {
				query: "hmmerp.projects.doctype.project.project.get_users_for_project",
			};
		});
			...
			if (frappe.model.can_read("Task")) {
				...
				frm.add_custom_button(
					...
					() => {
						frappe
							.call(
								"hmmerp.projects.doctype.project.project.create_kanban_board_if_not_exists",
								{
									project: frm.doc.name,
								}
							)
	...
	update_costing_and_billing: function (frm) {
		frappe.call({
			method: "hmmerp.projects.doctype.project.project.update_costing_and_billing",
	...
	create_duplicate: function (frm) {
		return new Promise((resolve) => {
			frappe.prompt("Project Name", (data) => {
				frappe
					.xcall("hmmerp.projects.doctype.project.project.create_duplicate_project", {
						prev_doc: frm.doc,
	...
	set_status: function (frm, status) {
		frappe.confirm(__("Set Project and all Tasks to status {0}?", [__(status).bold()]), () => {
			frappe
				.xcall("hmmerp.projects.doctype.project.project.set_project_status", {
					project: frm.doc.name,
					status: status,
				})
				.then(() => {
