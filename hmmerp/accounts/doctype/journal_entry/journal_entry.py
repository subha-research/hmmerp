...
import hmmerp
from hmmerp.accounts.deferred_revenue import get_deferred_booking_accounts
...
class JournalEntry(AccountsController):
	...
	if TYPE_CHECKING:
		...
		from hmmerp.accounts.doctype.journal_entry_account.journal_entry_account import JournalEntryAccount
	...
	def apply_tax_withholding(self):
		from hmmerp.accounts.report.general_ledger.general_ledger import get_account_type_map
				...
				if not blank_row:
					blank_row = self.append(
						"accounts",
						{
							...
							"cost_center": hmmerp.get_default_cost_center(self.company),
						},
					)
...
def get_outstanding(args):
	...
	company_currency = hmmerp.get_company_currency(args.get("company"))
	due_date = None
...
def get_account_details_and_party_type(account, date, company, debit=None, credit=None, exchange_rate=None):
	...
	company_currency = hmmerp.get_company_currency(company)
	account_details = frappe.get_cached_value(
