...
frappe.ui.form.on("Bank Reconciliation Tool", {
	...
	refresh: function (frm) {
		...
		frm.add_custom_button(__("Upload Bank Statement"), () =>
			frappe.call({
				method: "hmmerp.accounts.doctype.bank_statement_import.bank_statement_import.upload_bank_statement",
				args: {
					dt: frm.doc.doctype,
					dn: frm.doc.name,
					company: frm.doc.company,
		...
		frm.add_custom_button(__("Auto Reconcile"), function () {
			...
			frappe.call({
				method: "hmmerp.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool.auto_reconcile_vouchers",
				args: {
					bank_account: frm.doc.bank_account,
	...
	get_account_opening_balance(frm) {
		if (frm.doc.company && frm.doc.bank_account && frm.doc.bank_statement_from_date) {
			frappe.call({
				method: "hmmerp.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool.get_account_balance",
				args: {
					bank_account: frm.doc.bank_account,
	...
	render_chart(frm) {
		frm.cards_manager = new hmmerp.accounts.bank_reconciliation.NumberCardManager({
	...
	render(frm) {
		if (frm.doc.bank_account) {
			frm.bank_reconciliation_data_table_manager =
				new hmmerp.accounts.bank_reconciliation.DataTableManager({
					company: frm.doc.company,
					bank_account: frm.doc.bank_account,
