...
hmmerp.accounts.taxes.setup_tax_validations("Payment Entry");
hmmerp.accounts.taxes.setup_tax_filters("Advance Taxes and Charges");
...
frappe.ui.form.on("Payment Entry", {
	...
	setup: function (frm) {
		...
		frm.set_query("payment_term", "references", function (frm, cdt, cdn) {
			...
			if (
				...
				return {
					query: "hmmerp.controllers.queries.get_payment_terms_for_references",
					filters: {
		...
		frm.set_query("payment_request", "references", function (doc, cdt, cdn) {
			...
			return {
				query: "hmmerp.accounts.doctype.payment_request.payment_request.get_open_payment_requests_query",
				filters: {
	...
	refresh: function (frm) {
		hmmerp.hide_company(frm);
		frm.events.hide_unhide_fields(frm);
		frm.events.set_dynamic_labels(frm);
		frm.events.show_general_ledger(frm);
		hmmerp.accounts.ledger_preview.show_accounting_ledger_preview(frm);
		if (
			frm.doc.references &&
	...
	party: function (frm) {
		...
		if (frm.doc.payment_type && frm.doc.party_type && frm.doc.party && frm.doc.company) {
			...
			return frappe.call({
				method: "hmmerp.accounts.doctype.payment_entry.payment_entry.get_party_details",
				args: {
					company: frm.doc.company,
					party_type: frm.doc.party_type,
					party: frm.doc.party,
	...
	set_account_currency_and_balance: function (frm, account, currency_field, callback_function) {
		...
		if (frm.doc.posting_date && account) {
			frappe.call({
				method: "hmmerp.accounts.doctype.payment_entry.payment_entry.get_account_details",
				args: {
					account: account,
					date: frm.doc.posting_date,
					cost_center: frm.doc.cost_center,
	...
	source_exchange_rate: function (frm) {
		...
		frm.set_df_property("source_exchange_rate", "read_only", hmmerp.stale_rate_allowed() ? 0 : 1);
	},
	...
	target_exchange_rate: function (frm) {
		...
		// Make read only if Accounts Settings doesn't allow stale rates
		frm.set_df_property("target_exchange_rate", "read_only", hmmerp.stale_rate_allowed() ? 0 : 1);
	},
	...
	get_outstanding_documents: function (frm, filters, get_outstanding_invoices, get_orders_to_be_billed) {
		...
		return frappe.call({
			method: "hmmerp.accounts.doctype.payment_entry.payment_entry.get_outstanding_reference_documents",
			args: {
				args: args,
			},
			callback: function (r, rt) {
	...
	bank_account: function (frm) {
		...
		if (frm.doc.bank_account && ["Pay", "Receive"].includes(frm.doc.payment_type)) {
			frappe.call({
				method: "hmmerp.accounts.doctype.bank_account.bank_account.get_bank_account_details",
				args: {
...
frappe.ui.form.on("Payment Entry Reference", {
	...
	reference_name: function (frm, cdt, cdn) {
		...
		if (row.reference_name && row.reference_doctype) {
			return frappe.call({
				method: "hmmerp.accounts.doctype.payment_entry.payment_entry.get_reference_details",
				args: {
					reference_doctype: row.reference_doctype,
					reference_name: row.reference_name,
					party_account_currency:
...
function get_company_defaults(company) {
	return frappe.call({
		method: "hmmerp.accounts.doctype.payment_entry.payment_entry.get_company_defaults",
		args: {
			company: company,
		},
	});
