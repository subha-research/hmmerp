...
from hmmerp.accounts.utils import _delete_adv_pl_entries, _delete_pl_entries, create_payment_ledger_entry
...
class RepostPaymentLedger(Document):
	...
	if TYPE_CHECKING:
		...
		from hmmerp.accounts.doctype.repost_payment_ledger_items.repost_payment_ledger_items import (
...
def execute_repost_payment_ledger(docname):
	...
	frappe.enqueue(
		method="hmmerp.accounts.doctype.repost_payment_ledger.repost_payment_ledger.start_payment_ledger_repost",
		docname=docname,
		is_async=True,
		job_name=job_name,
	)
