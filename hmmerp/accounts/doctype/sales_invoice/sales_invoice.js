...
frappe.provide("hmmerp.accounts");

cur_frm.cscript.tax_table = "Sales Taxes and Charges";

hmmerp.accounts.taxes.setup_tax_validations("Sales Invoice");
hmmerp.accounts.payment_triggers.setup("Sales Invoice");
hmmerp.accounts.pos.setup("Sales Invoice");
hmmerp.accounts.taxes.setup_tax_filters("Sales Taxes and Charges");
hmmerp.sales_common.setup_selling_controller();
hmmerp.accounts.SalesInvoiceController = class SalesInvoiceController extends (
	hmmerp.selling.SellingController
) {
	setup(doc) {
	...
	onload() {
		...
		hmmerp.queries.setup_queries(this.frm, "Warehouse", function () {
			return hmmerp.queries.warehouse(me.frm.doc);
		});
		...
		hmmerp.queries.setup_warehouse_query(this.frm);
	}
	...
	refresh(doc, dt, dn) {
		...
		hmmerp.accounts.unreconcile_payment.add_unreconcile_btn(me.frm);
	...
	make_invoice_discounting() {
		frappe.model.open_mapped_doc({
			method: "hmmerp.accounts.doctype.sales_invoice.sales_invoice.create_invoice_discounting",
			frm: this.frm,
		});
	}
	...
	make_dunning() {
		frappe.model.open_mapped_doc({
			method: "hmmerp.accounts.doctype.sales_invoice.sales_invoice.create_dunning",
			frm: this.frm,
		});
	}
	...
	make_maintenance_schedule() {
		frappe.model.open_mapped_doc({
			method: "hmmerp.accounts.doctype.sales_invoice.sales_invoice.make_maintenance_schedule",
			frm: this.frm,
		});
	}
		...
		this.$sales_order_btn = this.frm.add_custom_button(
			...
			function () {
				hmmerp.utils.map_current_doc({
					method: "hmmerp.selling.doctype.sales_order.sales_order.make_sales_invoice",
	...
	quotation_btn() {
		...
		this.$quotation_btn = this.frm.add_custom_button(
			...
			function () {
				hmmerp.utils.map_current_doc({
					method: "hmmerp.selling.doctype.quotation.quotation.make_sales_invoice",
			...
			function () {
				hmmerp.utils.map_current_doc({
					...
					get_query: function () {
						...
						return {
							query: "hmmerp.controllers.queries.get_delivery_notes_to_be_billed",
							filters: filters,
						};
					},
				});
	...
	customer() {
		...
		hmmerp.utils.get_party_details(
			this.frm,
			"hmmerp.accounts.party.get_party_details",
			{
		...
		if (this.frm.doc.customer) {
			frappe.call({
				method: "hmmerp.accounts.doctype.sales_invoice.sales_invoice.get_loyalty_programs",
				args: {
	...
	make_inter_company_invoice() {
		let me = this;
		frappe.model.open_mapped_doc({
			method: "hmmerp.accounts.doctype.sales_invoice.sales_invoice.make_inter_company_purchase_invoice",
			frm: me.frm,
		});
	}
	...
	make_sales_return() {
		frappe.model.open_mapped_doc({
			method: "hmmerp.accounts.doctype.sales_invoice.sales_invoice.make_sales_return",
			frm: this.frm,
		});
	}
	...
	asset(frm, cdt, cdn) {
		...
		if (row.asset) {
			frappe.call({
				method: hmmerp.assets.doctype.asset.depreciation.get_disposal_account_and_cost_center,
				args: {
...
extend_cscript(cur_frm.cscript, new hmmerp.accounts.SalesInvoiceController({ frm: cur_frm }));

cur_frm.cscript["Make Delivery Note"] = function () {
	frappe.model.open_mapped_doc({
		method: "hmmerp.accounts.doctype.sales_invoice.sales_invoice.make_delivery_note",
		frm: cur_frm,
	});
};

cur_frm.cscript.income_account = function (doc, cdt, cdn) {
	hmmerp.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "income_account");
};

cur_frm.cscript.expense_account = function (doc, cdt, cdn) {
	hmmerp.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "expense_account");
};

cur_frm.cscript.cost_center = function (doc, cdt, cdn) {
	hmmerp.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "cost_center");
};
...
frappe.ui.form.on("Sales Invoice", {
	setup: function (frm) {
		...
		frm.set_query("income_account", "items", function (doc) {
			return {
				query: "hmmerp.controllers.queries.get_income_account",
				filters: {
					company: doc.company,
					disabled: 0,
				},
		...
		frm.set_query("time_sheet", "timesheets", function (doc, cdt, cdn) {
			return {
				query: "hmmerp.projects.doctype.timesheet.timesheet.get_timesheet",
		...
		frm.set_query("pos_profile", function (doc) {
			...
			return {
				query: "hmmerp.accounts.doctype.pos_profile.pos_profile.pos_profile_query",
				filters: {
					company: doc.company,
				},
			};
	...
	loyalty_points: function (frm) {
		if (frm.redemption_conversion_factor) {
			...
			frappe.call({
				method: "hmmerp.accounts.doctype.loyalty_program.loyalty_program.get_redeemption_factor",
				args: {
	...
	get_loyalty_details: function (frm) {
		if (frm.doc.customer && frm.doc.redeem_loyalty_points) {
			frappe.call({
				method: "hmmerp.accounts.doctype.loyalty_program.loyalty_program.get_loyalty_program_details",
				args: {
	...
	async get_timesheet_data(frm, kwargs) {
		return frappe
			.call({
				method: "hmmerp.projects.doctype.timesheet.timesheet.get_projectwise_timesheet_data",
				args: kwargs,
			})
			.then((r) => {
					...
					let d = new frappe.ui.Dialog({
						...
						fields: [
							...
							},
							{
								...
								get_query: () => {
									return {
										query: "hmmerp.controllers.queries.item_query",
										filters: {
											is_sales_item: 1,
											customer: frm.doc.customer,
											has_variants: 0,
...
var set_timesheet_detail_rate = function (cdt, cdn, currency, timelog) {
	frappe.call({
		method: "hmmerp.projects.doctype.timesheet.timesheet.get_timesheet_detail_rate",
		args: {
			timelog: timelog,
			currency: currency,
		},
