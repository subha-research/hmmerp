...
frappe.ui.form.on("Payment Request", "onload", function (frm, dt, dn) {
	if (frm.doc.reference_doctype) {
		frappe.call({
			method: "hmmerp.accounts.doctype.payment_request.payment_request.get_print_format_list",
...
frappe.ui.form.on("Payment Request", "refresh", function (frm) {
	...
	if (
		...
		frm.add_custom_button(__("Resend Payment Email"), function () {
			frappe.call({
				method: "hmmerp.accounts.doctype.payment_request.payment_request.resend_payment_email",
	...
	if (
		...
		frm.add_custom_button(__("Create Payment Entry"), function () {
			frappe.call({
				method: "hmmerp.accounts.doctype.payment_request.payment_request.make_payment_entry",
				args: { docname: frm.doc.name },
				freeze: true,
				callback: function (r) {
					if (!r.exc) {
...
frappe.ui.form.on("Payment Request", "is_a_subscription", function (frm) {
	...
	if (frm.doc.is_a_subscription && frm.doc.reference_doctype && frm.doc.reference_name) {
		frappe.call({
			method: "hmmerp.accounts.doctype.payment_request.payment_request.get_subscription_details",
