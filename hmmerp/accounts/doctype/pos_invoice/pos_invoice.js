...
hmmerp.accounts.pos.setup("POS Invoice");
hmmerp.selling.POSInvoiceController = class POSInvoiceController extends hmmerp.selling.SellingController {
	settings = {};
	...
	onload(doc) {
		...
		this.frm.set_query("item_code", "items", function (doc) {
			return {
				query: "hmmerp.accounts.doctype.pos_invoice.pos_invoice.item_query",
				filters: {
					has_variants: ["=", 0],
					is_sales_item: ["=", 1],
					disabled: ["=", 0],
	...
	make_sales_return() {
		frappe.model.open_mapped_doc({
			method: "hmmerp.accounts.doctype.pos_invoice.pos_invoice.make_sales_return",
			frm: this.frm,
		});
	}
...
extend_cscript(cur_frm.cscript, new hmmerp.selling.POSInvoiceController({ frm: cur_frm }));
