	...
	refresh: function (frm) {
		if (frm.doc.docstatus == 1 && ["Queued", "Paused"].find((x) => x == frm.doc.status)) {
			...
			frm.add_custom_button(execute_btn, () => {
				frm.call({
					method: "hmmerp.accounts.doctype.process_payment_reconciliation.process_payment_reconciliation.trigger_job_for_doc",
					args: {
		...
		if (
			...
			frm.call({
				method: "hmmerp.accounts.doctype.process_payment_reconciliation.process_payment_reconciliation.get_reconciled_count",
				args: {
					docname: frm.docname,
				},
			}).then((r) => {
		...
		if (frm.doc.docstatus == 1 && frm.doc.status == "Running") {
			...
			frm.add_custom_button(execute_btn, () => {
				frm.call({
					method: "hmmerp.accounts.doctype.process_payment_reconciliation.process_payment_reconciliation.pause_job_for_doc",
					args: {
						docname: frm.docname,
					},
				}).then((r) => {
