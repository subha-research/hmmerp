...
frappe.ui.form.on("Dunning", {
	setup: function (frm) {
		...
		frm.set_query("company_address", hmmerp.queries.company_address_query);
	...
	conversion_rate: function (frm) {
		if (frm.doc.currency === hmmerp.get_currency(frm.doc.company)) {
			frm.set_value("conversion_rate", 1.0);
		}

		// Make read only if Accounts Settings doesn't allow stale rates
		frm.set_df_property("conversion_rate", "read_only", hmmerp.stale_rate_allowed() ? 0 : 1);
	},
	customer_address: function (frm) {
		hmmerp.utils.get_address_display(frm, "customer_address");
	},
	company_address: function (frm) {
		hmmerp.utils.get_address_display(frm, "company_address");
	},
	...
	get_dunning_letter_text: function (frm) {
		if (frm.doc.dunning_type) {
			frappe.call({
				method: "hmmerp.accounts.doctype.dunning.dunning.get_dunning_letter_text",
				args: {
	...
	make_payment_entry: function (frm) {
		return frappe.call({
			method: "hmmerp.accounts.doctype.payment_entry.payment_entry.get_payment_entry",
			args: {
				dt: frm.doc.doctype,
				dn: frm.doc.name,
			},
