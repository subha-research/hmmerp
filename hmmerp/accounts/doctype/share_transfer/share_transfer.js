...
frappe.provide("hmmerp.share_transfer");
...
frappe.ui.form.on("Share Transfer", {
	refresh: function (frm) {
		...
		if (frm.doc.docstatus == 1 && frm.doc.equity_or_liability_account && frm.doc.asset_account) {
			frm.add_custom_button(__("Create Journal Entry"), function () {
				hmmerp.share_transfer.make_jv(frm);
			});
		}
	...
	no_of_shares: (frm) => {
		if (frm.doc.rate != undefined || frm.doc.rate != null) {
			hmmerp.share_transfer.update_amount(frm);
		}
	},
	rate: (frm) => {
...
hmmerp.share_transfer.update_amount = function (frm) {
	frm.doc.amount = frm.doc.no_of_shares * frm.doc.rate;
	frm.refresh_field("amount");
};

hmmerp.share_transfer.make_jv = function (frm) {
	var account,
		payment_account,
	...
	frappe.call({
		...
		method: "hmmerp.accounts.doctype.share_transfer.share_transfer.make_jv_entry",
		callback: function (r) {
