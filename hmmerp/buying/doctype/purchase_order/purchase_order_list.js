frappe.listview_settings["Purchase Order"] = {
	...
	onload: function (listview) {
		var method = "hmmerp.buying.doctype.purchase_order.purchase_order.close_or_unclose_purchase_orders";
		...
		if (frappe.model.can_create("Purchase Invoice")) {
			listview.page.add_action_item(__("Purchase Invoice"), () => {
				hmmerp.bulk_transaction_processing.create(listview, "Purchase Order", "Purchase Invoice");
			});
		}

		if (frappe.model.can_create("Purchase Receipt")) {
			listview.page.add_action_item(__("Purchase Receipt"), () => {
				hmmerp.bulk_transaction_processing.create(listview, "Purchase Order", "Purchase Receipt");
			});
		}

		if (frappe.model.can_create("Payment Entry")) {
			listview.page.add_action_item(__("Advance Payment"), () => {
				hmmerp.bulk_transaction_processing.create(listview, "Purchase Order", "Payment Entry");
			});
		}
	},
};
