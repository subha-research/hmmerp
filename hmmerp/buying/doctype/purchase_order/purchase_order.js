...
frappe.provide("hmmerp.buying");
frappe.provide("hmmerp.accounts.dimensions");

cur_frm.cscript.tax_table = "Purchase Taxes and Charges";

hmmerp.accounts.taxes.setup_tax_filters("Purchase Taxes and Charges");
hmmerp.accounts.taxes.setup_tax_validations("Purchase Order");
hmmerp.buying.setup_buying_controller();
...
frappe.ui.form.on("Purchase Order", {
	setup: function (frm) {
		...
		frm.set_query("expense_account", "items", function () {
			return {
				query: "hmmerp.controllers.queries.get_expense_account",
	...
	get_materials_from_supplier: function (frm) {
		...
		if (po_details && po_details.length) {
			frm.add_custom_button(
				...
				() => {
					frm.call({
						method: "hmmerp.controllers.subcontracting_controller.get_materials_from_supplier",
						freeze: true,
	...
	onload: function (frm) {
		...
		hmmerp.queries.setup_queries(frm, "Warehouse", function () {
			return hmmerp.queries.warehouse(frm.doc);
		});
	...
	get_subcontracting_boms_for_finished_goods: function (fg_item) {
		return frappe.call({
			method: "hmmerp.subcontracting.doctype.subcontracting_bom.subcontracting_bom.get_subcontracting_boms_for_finished_goods",
			args: {
				fg_items: fg_item,
			},
		});
	...
	get_subcontracting_boms_for_service_item: function (service_item) {
		return frappe.call({
			method: "hmmerp.subcontracting.doctype.subcontracting_bom.subcontracting_bom.get_subcontracting_boms_for_service_item",
			args: {
...
frappe.ui.form.on("Purchase Order Item", {
	schedule_date: function (frm, cdt, cdn) {
		...
		if (row.schedule_date) {
			if (!frm.doc.schedule_date) {
				hmmerp.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "schedule_date");
			} else {
				set_schedule_date(frm);
			}
		}
...
hmmerp.buying.PurchaseOrderController = class PurchaseOrderController extends (
	hmmerp.buying.BuyingController
) {
	setup() {
				...
				if (
					...
					if (!this.frm.doc.__onload || this.frm.doc.__onload.can_update_items) {
						this.frm.add_custom_button(__("Update Items"), () => {
							hmmerp.utils.update_child_items({
								frm: this.frm,
								child_docname: "items",
	...
	get_items_from_open_material_requests() {
		hmmerp.utils.map_current_doc({
			method: "hmmerp.stock.doctype.material_request.material_request.make_purchase_order_based_on_supplier",
			args: {
			...
			get_query_method:
				"hmmerp.stock.doctype.material_request.material_request.get_material_requests_based_on_supplier",
		});
	}
	...
	make_stock_entry() {
		frappe.call({
			method: "hmmerp.controllers.subcontracting_controller.make_rm_stock_entry",
			args: {
	...
	make_inter_company_order(frm) {
		frappe.model.open_mapped_doc({
			method: "hmmerp.buying.doctype.purchase_order.purchase_order.make_inter_company_sales_order",
			frm: frm,
		});
	}
	...
	make_purchase_receipt() {
		frappe.model.open_mapped_doc({
			method: "hmmerp.buying.doctype.purchase_order.purchase_order.make_purchase_receipt",
			frm: this.frm,
	...
	make_purchase_invoice() {
		frappe.model.open_mapped_doc({
			method: "hmmerp.buying.doctype.purchase_order.purchase_order.make_purchase_invoice",
			frm: this.frm,
		});
	}
	...
	make_subcontracting_order() {
		frappe.model.open_mapped_doc({
			method: "hmmerp.buying.doctype.purchase_order.purchase_order.make_subcontracting_order",
			frm: this.frm,
	...
	add_from_mappers() {
		...
		this.frm.add_custom_button(
			...
			function () {
				hmmerp.utils.map_current_doc({
					method: "hmmerp.stock.doctype.material_request.material_request.make_purchase_order",
					source_doctype: "Material Request",
					target: me.frm,
					setters: {
		...
		this.frm.add_custom_button(
			...
			function () {
				hmmerp.utils.map_current_doc({
					method: "hmmerp.buying.doctype.supplier_quotation.supplier_quotation.make_purchase_order",
		...
		this.frm.add_custom_button(
			...
			function () {
				...
				frappe.call({
					method: "hmmerp.buying.utils.get_linked_material_requests",
					args: {
						items: my_items,
					},
					callback: function (r) {
...
extend_cscript(cur_frm.cscript, new hmmerp.buying.PurchaseOrderController({ frm: cur_frm }));

cur_frm.cscript.update_status = function (label, status) {
	frappe.call({
		method: "hmmerp.buying.doctype.purchase_order.purchase_order.update_status",
...
function set_schedule_date(frm) {
	if (frm.doc.schedule_date) {
		hmmerp.utils.copy_value_in_all_rows(
			frm.doc,
			frm.doc.doctype,
			frm.doc.name,
			"items",
...
frappe.ui.form.on("Purchase Order", "is_subcontracted", function (frm) {
	if (frm.doc.is_old_subcontracting_flow) {
		hmmerp.buying.get_default_bom(frm);
	}
});
