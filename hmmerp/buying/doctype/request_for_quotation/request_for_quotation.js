	...
	refresh: function (frm, cdt, cdn) {
		if (frm.doc.docstatus === 1) {
			...
			frm.add_custom_button(
				...
				function () {
					frappe.call({
						method: "hmmerp.buying.doctype.request_for_quotation.request_for_quotation.send_supplier_emails",
						freeze: true,
						args: {
						...
						(data) => {
							var w = window.open(
								frappe.urllib.get_full_url(
									"/api/method/hmmerp.buying.doctype.request_for_quotation.request_for_quotation.get_pdf?" +
										new URLSearchParams({
											name: frm.doc.name,
	...
	make_supplier_quotation: function (frm) {
		...
		var dialog = new frappe.ui.Dialog({
			...
			primary_action: (args) => {
				...
				return frappe.call({
					type: "GET",
					method: "hmmerp.buying.doctype.request_for_quotation.request_for_quotation.make_supplier_quotation_from_rfq",
					args: {
...
frappe.ui.form.on("Request for Quotation Supplier", {
	supplier: function (frm, cdt, cdn) {
		...
		frappe.call({
			method: "hmmerp.accounts.party.get_party_details",
			args: {
				party: d.supplier,
				party_type: "Supplier",
			},
...
hmmerp.buying.RequestforQuotationController = class RequestforQuotationController extends (
	...
	refresh() {
		...
		if (this.frm.doc.docstatus === 0) {
				...
				function () {
					...
					var dialog = new frappe.ui.Dialog({
						...
						primary_action: (args) => {
							...
							hmmerp.utils.map_current_doc({
								method: "hmmerp.buying.doctype.request_for_quotation.request_for_quotation.get_item_from_material_requests_based_on_supplier",
			...
			this.frm.add_custom_button(
				...
				function () {
					hmmerp.buying.link_to_mrs(me.frm);
				},
				__("Tools")
			);
			...
			fields: [
				{
					...
					onchange() {
						if (dialog.get_value("search_type") == "Tag") {
							frappe
								.call({
									method: "hmmerp.buying.doctype.request_for_quotation.request_for_quotation.get_supplier_tag",
								})
								.then((r) => {
...
extend_cscript(cur_frm.cscript, new hmmerp.buying.RequestforQuotationController({ frm: cur_frm }));
