	...
	filters: [
		...
		},
		{
			...
			get_query: () => {
				...
				if (quote != "") {
					return {
						query: "hmmerp.stock.doctype.quality_inspection.quality_inspection.item_query",
						filters: {
	...
	make_default_supplier_dialog: (report) => {
		...
		dialog.set_primary_action(__("Set Default Supplier"), () => {
			...
			if (values) {
				...
				frappe.call({
					method: "hmmerp.buying.report.supplier_quotation_comparison.supplier_quotation_comparison.set_default_supplier",
					args: {
