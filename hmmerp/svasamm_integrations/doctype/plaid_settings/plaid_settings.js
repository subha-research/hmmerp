...
frappe.provide("hmmerp.integrations");
	...
	refresh: function (frm) {
		if (frm.doc.enabled) {
			frm.add_custom_button(__("Link a new bank account"), () => {
				new hmmerp.integrations.plaidLink(frm);
			});
			...
			frm.add_custom_button(__("Sync Now"), () => {
				frappe.call({
					method: "hmmerp.hmmerp_integrations.doctype.plaid_settings.plaid_settings.enqueue_synchronization",
					freeze: true,
					callback: () => {
...
hmmerp.integrations.plaidLink = class plaidLink {
	constructor(parent) {
		this.frm = parent;
	...
	plaid_success(token, response) {
		...
		frappe.prompt(
			...
			(data) => {
				...
				frappe
					.xcall(
						"hmmerp.hmmerp_integrations.doctype.plaid_settings.plaid_settings.add_institution",
						{
							token: token,
							response: response,
						}
					...
					.then((result) => {
						frappe.xcall(
							"hmmerp.hmmerp_integrations.doctype.plaid_settings.plaid_settings.add_bank_accounts",
							{
								response: response,
								bank: result,
								company: me.company,
