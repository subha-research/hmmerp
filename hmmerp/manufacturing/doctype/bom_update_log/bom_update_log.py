...
from hmmerp.manufacturing.doctype.bom_update_log.bom_updation_utils import (
	get_leaf_boms,
...
class BOMUpdateLog(Document):
	...
	if TYPE_CHECKING:
		...
		from hmmerp.manufacturing.doctype.bom_update_batch.bom_update_batch import BOMUpdateBatch
	...
	def on_submit(self):
		if self.update_type == "Replace BOM":
			...
			frappe.enqueue(
				method="hmmerp.manufacturing.doctype.bom_update_log.bom_update_log.run_replace_bom_job",
				doc=self,
				boms=boms,
				timeout=40000,
				now=frappe.in_test,
			...
			frappe.enqueue(
				method="hmmerp.manufacturing.doctype.bom_update_log.bom_update_log.process_boms_cost_level_wise",
				queue="long",
				update_doc=self,
				now=frappe.in_test,
				enqueue_after_commit=True,
...
def queue_bom_cost_jobs(current_boms_list: list[str], update_doc: "BOMUpdateLog", current_level: int) -> None:
	...
	while current_boms_list:
		...
		frappe.enqueue(
			method="hmmerp.manufacturing.doctype.bom_update_log.bom_updation_utils.update_cost_in_level",
			doc=update_doc,
