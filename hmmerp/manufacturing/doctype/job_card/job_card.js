...
frappe.ui.form.on("Job Card", {
	setup: function (frm) {
		frm.set_query("operation", function () {
			return {
				query: "hmmerp.manufacturing.doctype.job_card.job_card.get_operations",
				filters: {
		...
		if (frm.doc.docstatus === 1 && frm.doc.for_quantity > frm.doc.manufactured_qty) {
			frm.add_custom_button(__("Make Subcontracting PO"), () => {
				frappe.model.open_mapped_doc({
					method: "hmmerp.manufacturing.doctype.job_card.job_card.make_subcontracting_po",
					frm: frm,
				});
			}).addClass("btn-primary");
		}
	...
	make_corrective_job_card: function (frm, operation, for_operation) {
		frappe.call({
			method: "hmmerp.manufacturing.doctype.job_card.job_card.make_corrective_job_card",
			args: {
	...
	operation: function (frm) {
		...
		if (frm.doc.operation && frm.doc.work_order) {
			frappe.call({
				method: "hmmerp.manufacturing.doctype.job_card.job_card.get_operation_details",
				args: {
					work_order: frm.doc.work_order,
	...
	make_time_log: function (frm, args) {
		...
		frappe.call({
			method: "hmmerp.manufacturing.doctype.job_card.job_card.make_time_log",
			args: {
				args: args,
			},
			freeze: true,
	...
	make_material_request: function (frm) {
		frappe.model.open_mapped_doc({
			method: "hmmerp.manufacturing.doctype.job_card.job_card.make_material_request",
			frm: frm,
	...
	make_stock_entry: function (frm) {
		frappe.model.open_mapped_doc({
			method: "hmmerp.manufacturing.doctype.job_card.job_card.make_stock_entry",
			frm: frm,
			run_link_triggers: true,
		});
	},
