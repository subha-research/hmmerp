...
frappe.ui.form.on("Production Plan", {
	...
	setup_queries(frm) {
		frm.set_query("sales_order", "sales_orders", () => {
			return {
				query: "hmmerp.manufacturing.doctype.production_plan.production_plan.sales_order_query",
				filters: {
		...
		const projected_qty_formula = ` <table class="table table-bordered" style="background-color: var(--scrollbar-track-color);">
			<tr><td style="padding-left:25px">
				...
				<h3 style="text-decoration: underline;">
					<a href = "https://hmmerp.com/docs/user/manual/en/stock/projected-quantity">
		...
		if (frm.doc.docstatus === 1 && frm.doc.reserve_stock) {
			if (frm.events.has_unreserved_stock(frm, "sub_assembly_items")) {
				frm.add_custom_button(
					...
					() => hmmerp.stock_reservation.make_entries(frm, "sub_assembly_items"),
			...
			if (frm.events.has_reserved_stock(frm, "sub_assembly_items")) {
				...
				frm.add_custom_button(
					...
					() => hmmerp.stock_reservation.show_reserved_stock(frm, "sub_assembly_items"),
					__("Stock Reservation")
				);
			}
		}
		...
		if (frm.doc.docstatus === 1 && frm.doc.reserve_stock) {
			if (frm.events.has_unreserved_stock(frm, "mr_items", "required_bom_qty")) {
				frm.add_custom_button(
					...
					() => hmmerp.stock_reservation.make_entries(frm, "mr_items"),
					__("Stock Reservation")
				);
			}
			...
			if (frm.events.has_reserved_stock(frm, "mr_items")) {
				...
				frm.add_custom_button(
					...
					() => hmmerp.stock_reservation.show_reserved_stock(frm, "mr_items"),
					__("Stock Reservation")
				);
			}
		}
	...
	get_items_for_material_requests(frm, warehouses) {
		frappe.call({
			method: "hmmerp.manufacturing.doctype.production_plan.production_plan.get_items_for_material_requests",
			freeze: true,
			args: {
				doc: frm.doc,
		...
		frappe.prompt(
			...
			(row) => {
				let get_template_url =
					"hmmerp.manufacturing.doctype.production_plan.production_plan.download_raw_materials";
...
frappe.ui.form.on("Production Plan Item", {
	item_code(frm, cdt, cdn) {
		...
		if (row.item_code) {
			frappe.call({
				method: "hmmerp.manufacturing.doctype.production_plan.production_plan.get_item_data",
				args: {
...
frappe.ui.form.on("Material Request Plan Item", {
	warehouse(frm, cdt, cdn) {
		...
		if (row.warehouse && row.item_code && frm.doc.company) {
			frappe.call({
				method: "hmmerp.manufacturing.doctype.production_plan.production_plan.get_bin_details",
				args: {
					row: row,
					company: frm.doc.company,
	...
	sales_order(frm, cdt, cdn) {
		...
		if (row.sales_order) {
			frm.call({
				...
				callback(r) {
					frappe.call({
						method: "hmmerp.manufacturing.doctype.production_plan.production_plan.get_so_details",
						args: { sales_order },
						callback(r) {
...
frappe.ui.form.on("Production Plan Sub Assembly Item", {
	fg_warehouse(frm, cdt, cdn) {
		hmmerp.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "sub_assembly_items", "fg_warehouse");
