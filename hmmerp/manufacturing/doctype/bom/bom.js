...
frappe.ui.form.on("BOM", {
	setup(frm) {
		...
		frm.set_query("item", function () {
			return {
				query: "hmmerp.manufacturing.doctype.bom.bom.item_query",
				filters: {
					is_stock_item: 1,
				},
			};
	...
	make_work_order(frm) {
		frm.events.setup_variant_prompt(
			...
			(frm, item, data, variant_items, use_multi_level_bom) => {
				frappe.call({
					method: "hmmerp.manufacturing.doctype.work_order.work_order.make_work_order",
					args: {
						bom_no: frm.doc.name,
						item: item,
						qty: data.qty || 0.0,
	...
	make_variant_bom(frm) {
		frm.events.setup_variant_prompt(
			...
			(frm, item, data, variant_items) => {
				frappe.call({
					method: "hmmerp.manufacturing.doctype.bom.bom.make_variant_bom",
					args: {
	...
	make_quality_inspection(frm) {
		frappe.model.open_mapped_doc({
			method: "hmmerp.stock.doctype.quality_inspection.quality_inspection.make_quality_inspection",
			frm: frm,
		});
	},
		...
		if (frm.doc.routing) {
			frappe.call({
				...
				callback(r) {
					if (!r.exc) {
						frm.refresh_fields();
						hmmerp.bom.calculate_op_cost(frm.doc);
						hmmerp.bom.calculate_total(frm.doc);
					}
				},
			});
		}
...
hmmerp.bom.BomController = class BomController extends hmmerp.TransactionController {
	conversion_rate(doc) {
		if (this.frm.doc.currency === this.get_company_currency()) {
			this.frm.set_value("conversion_rate", 1.0);
		} else {
			hmmerp.bom.update_cost(doc);
		}
	}
...
extend_cscript(cur_frm.cscript, new hmmerp.bom.BomController({ frm: cur_frm }));

cur_frm.cscript.hour_rate = function (doc) {
	hmmerp.bom.calculate_op_cost(doc);
	hmmerp.bom.calculate_total(doc);
};
	...
	if (d.item_code) {
		return frappe.call({
			...
			callback: function (r) {
				...
				hmmerp.bom.calculate_rm_cost(doc);
				hmmerp.bom.calculate_scrap_materials_cost(doc);
				hmmerp.bom.calculate_total(doc);
			},
			freeze: true,
		});
...
hmmerp.bom.update_cost = function (doc) {
	hmmerp.bom.calculate_op_cost(doc);
	hmmerp.bom.calculate_rm_cost(doc);
	hmmerp.bom.calculate_scrap_materials_cost(doc);
	hmmerp.bom.calculate_total(doc);
...
hmmerp.bom.calculate_op_cost = function (doc) {
...
// rm : raw material
hmmerp.bom.calculate_rm_cost = function (doc) {
	var rm = doc.items || [];
	var total_rm_cost = 0;
...
// sm : scrap material
hmmerp.bom.calculate_scrap_materials_cost = function (doc) {
...
hmmerp.bom.calculate_total = function (doc) {
...
frappe.ui.form.on("BOM Item", "items_remove", function (frm) {
	hmmerp.bom.calculate_rm_cost(frm.doc);
	hmmerp.bom.calculate_total(frm.doc);
});
