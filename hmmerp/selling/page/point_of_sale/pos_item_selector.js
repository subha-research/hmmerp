...
hmmerp.PointOfSale.ItemSelector = class {
	...
	async load_items_data() {
		if (!this.item_group) {
			frappe.call({
				method: "hmmerp.selling.page.point_of_sale.point_of_sale.get_parent_item_group",
				async: false,
				callback: (r) => {
	...
	get_items({ start = 0, page_length = 40, search_term = "" }) {
		...
		return frappe.call({
			method: "hmmerp.selling.page.point_of_sale.point_of_sale.get_items",
			freeze: true,
		...
		this.item_group_field = frappe.ui.form.make_control({
			df: {
				...
				get_query: function () {
					...
					return {
						query: "hmmerp.selling.page.point_of_sale.point_of_sale.item_group_query",
						filters: {
