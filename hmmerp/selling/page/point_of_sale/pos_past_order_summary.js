hmmerp.PointOfSale.PastOrderSummary = class {
	...
	async get_item_html(doc, item_data) {
		...
		async function get_returned_qty() {
			const r = await frappe.call({
				method: "hmmerp.controllers.sales_and_purchase_return.get_invoice_item_returned_qty",
				args: {
					doctype: doc.doctype,
					invoice: doc.name,
	...
	async is_invoice_returnable(doctype, invoice) {
		const r = await frappe.call({
			method: "hmmerp.controllers.sales_and_purchase_return.is_invoice_returnable",
			args: {
				doctype: doctype,
				invoice: invoice,
			},
