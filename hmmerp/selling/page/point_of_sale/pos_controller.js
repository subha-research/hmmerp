hmmerp.PointOfSale.Controller = class {
	constructor(wrapper) {
	...
	fetch_opening_entry() {
		return frappe.call("hmmerp.selling.page.point_of_sale.point_of_sale.check_opening_entry", {
	...
	create_opening_voucher() {
		...
		const dialog = new frappe.ui.Dialog({
			...
			primary_action: async function ({ company, pos_profile, balance_details }) {
				...
				const method = "hmmerp.selling.page.point_of_sale.point_of_sale.create_opening_voucher";
	...
	async prepare_app_defaults(data) {
		...
		frappe.call({
			method: "hmmerp.selling.page.point_of_sale.point_of_sale.get_pos_profile_data",
	...
	init_item_selector() {
		this.item_selector = new hmmerp.PointOfSale.ItemSelector({
	...
	init_item_cart() {
		this.cart = new hmmerp.PointOfSale.ItemCart({
			wrapper: this.$components_wrapper,
			settings: this.settings,
			events: {
				get_frm: () => this.frm,
	...
	init_item_details() {
		this.item_details = new hmmerp.PointOfSale.ItemDetails({
			wrapper: this.$components_wrapper,
			settings: this.settings,
			events: {
				get_frm: () => this.frm,
	...
	init_payments() {
		this.payment = new hmmerp.PointOfSale.Payment({
			wrapper: this.$components_wrapper,
			settings: this.settings,
			events: {
				get_frm: () => this.frm || {},
	...
	init_recent_order_list() {
		this.recent_order_list = new hmmerp.PointOfSale.PastOrderList({
			wrapper: this.$components_wrapper,
			events: {
	...
	init_order_summary() {
		this.order_summary = new hmmerp.PointOfSale.PastOrderSummary({
			wrapper: this.$components_wrapper,
			settings: this.settings,
			events: {
				get_frm: () => this.frm,
		...
		return frappe.call({
			method:
				doc.doctype == "POS Invoice"
					? "hmmerp.accounts.doctype.pos_invoice.pos_invoice.make_sales_return"
					: "hmmerp.accounts.doctype.sales_invoice.sales_invoice.make_sales_return",
			args: {
	...
	async check_serial_no_availablilty(item_code, warehouse, serial_no) {
		const method = "hmmerp.stock.doctype.serial_no.serial_no.get_pos_reserved_serial_nos";
	...
	get_available_stock(item_code, warehouse) {
		const me = this;
		return frappe.call({
			method: "hmmerp.accounts.doctype.pos_invoice.pos_invoice.get_stock_availability",
			args: {
				item_code: item_code,
				warehouse: warehouse,
			},
