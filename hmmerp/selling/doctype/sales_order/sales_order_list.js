frappe.listview_settings["Sales Order"] = {
	...
	onload: function (listview) {
		var method = "hmmerp.selling.doctype.sales_order.sales_order.close_or_unclose_sales_orders";
		...
		if (frappe.model.can_create("Sales Invoice")) {
			listview.page.add_action_item(__("Sales Invoice"), () => {
				hmmerp.bulk_transaction_processing.create(listview, "Sales Order", "Sales Invoice");
			});
		}
		...
		if (frappe.model.can_create("Delivery Note")) {
			listview.page.add_action_item(__("Delivery Note"), () => {
				frappe.call({
					method: "hmmerp.selling.doctype.sales_order.sales_order.is_enable_cutoff_date_on_bulk_delivery_note_creation",
					callback: (r) => {
						if (r.message) {
							...
							dialog.set_primary_action(__("Select"), function (values) {
								...
								hmmerp.bulk_transaction_processing.create(
									listview,
									"Sales Order",
									"Delivery Note",
									{
		...
		if (frappe.model.can_create("Payment Entry")) {
			listview.page.add_action_item(__("Advance Payment"), () => {
				hmmerp.bulk_transaction_processing.create(listview, "Sales Order", "Payment Entry");
			});
		}
	},
};
