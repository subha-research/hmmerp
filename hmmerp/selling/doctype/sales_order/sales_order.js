...
hmmerp.accounts.taxes.setup_tax_validations("Sales Order");
...
frappe.ui.form.on("Sales Order", {
	...
	refresh: function (frm) {
		...
		if (frm.doc.docstatus === 0) {
			...
			if (frm.doc.docstatus === 0) {
				frappe.call({
					method: "hmmerp.selling.doctype.sales_order.sales_order.get_stock_reservation_status",
					callback: function (r) {
						if (!r.message) {
	...
	onload: function (frm) {
		...
		frm.set_query("warehouse", "items", function (doc, cdt, cdn) {
			...
			if (row.item_code) {
				query.query = "hmmerp.controllers.queries.warehouse_query";
		...
		const dialog = new frappe.ui.Dialog({
			...
			fields: [
				...
				{ fieldtype: "Column Break" },
				{
					...
					get_query: () => {
						return {
							query: "hmmerp.controllers.queries.get_filtered_child_rows",
							filters: {
	...
	cancel_stock_reservation_entries(frm) {
		...
		frappe
			.call({
				method: "hmmerp.stock.doctype.stock_reservation_entry.stock_reservation_entry.get_stock_reservation_entries_for_voucher",
				args: {
...
frappe.ui.form.on("Sales Order Item", {
	...
	delivery_date: function (frm, cdt, cdn) {
		if (!frm.doc.delivery_date) {
			hmmerp.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "delivery_date");
		}
	},
});

hmmerp.selling.SalesOrderController = class SalesOrderController extends hmmerp.selling.SellingController {
	onload(doc, dt, dn) {
	...
	refresh(doc, dt, dn) {
		...
		if (this.frm.doc.docstatus === 0 && frappe.model.can_read("Quotation")) {
			this.frm.add_custom_button(
				__("Quotation"),
				function () {
					let d = hmmerp.utils.map_current_doc({
						method: "hmmerp.selling.doctype.quotation.quotation.make_sales_order",
	...
	create_pick_list() {
		frappe.model.open_mapped_doc({
			method: "hmmerp.selling.doctype.sales_order.sales_order.create_pick_list",
			frm: this.frm,
		});
	}
	...
	make_work_order() {
		var me = this;
		me.frm.call({
			method: "hmmerp.selling.doctype.sales_order.sales_order.get_work_order_items",
			args: {
	...
	make_material_request() {
		frappe.model.open_mapped_doc({
			method: "hmmerp.selling.doctype.sales_order.sales_order.make_material_request",
			frm: this.frm,
		});
	}
	...
	make_raw_material_request_dialog(r) {
		...
		var d = new frappe.ui.Dialog({
			...
			primary_action: function () {
				...
				me.frm.call({
					method: "hmmerp.selling.doctype.sales_order.sales_order.make_raw_material_request",
					args: {
						items: data,
						company: me.frm.doc.company,
						sales_order: me.frm.docname,
	...
	make_delivery_note(delivery_dates, for_reserved_stock = false) {
		frappe.model.open_mapped_doc({
			method: "hmmerp.selling.doctype.sales_order.sales_order.make_delivery_note",
			frm: this.frm,
			args: {
				delivery_dates,
	...
	make_maintenance_schedule() {
		frappe.model.open_mapped_doc({
			method: "hmmerp.selling.doctype.sales_order.sales_order.make_maintenance_schedule",
			frm: this.frm,
		});
	}
	...
	make_project() {
		frappe.model.open_mapped_doc({
			method: "hmmerp.selling.doctype.sales_order.sales_order.make_project",
			frm: this.frm,
		});
	}
	...
	make_inter_company_order() {
		frappe.model.open_mapped_doc({
			method: "hmmerp.selling.doctype.sales_order.sales_order.make_inter_company_purchase_order",
			frm: this.frm,
		});
	}
	...
	make_maintenance_visit() {
		frappe.model.open_mapped_doc({
			method: "hmmerp.selling.doctype.sales_order.sales_order.make_maintenance_visit",
			frm: this.frm,
		});
	}
		...
		var dialog = new frappe.ui.Dialog({
			...
			primary_action(args) {
				...
				var method = args.against_default_supplier
					...
					: "make_purchase_order";
				return frappe.call({
					method: "hmmerp.selling.doctype.sales_order.sales_order." + method,
	...
	update_status(label, status) {
		...
		frappe.call({
			method: "hmmerp.selling.doctype.sales_order.sales_order.update_status",
...
extend_cscript(cur_frm.cscript, new hmmerp.selling.SalesOrderController({ frm: cur_frm }));
