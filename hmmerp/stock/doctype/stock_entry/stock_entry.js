...
frappe.provide("hmmerp.stock");
frappe.provide("hmmerp.accounts.dimensions");

hmmerp.landed_cost_taxes_and_charges.setup_triggers("Stock Entry");
...
frappe.ui.form.on("Stock Entry", {
	setup: function (frm) {
		...
		frm.set_query("batch_no", "items", function (doc, cdt, cdn) {
			...
			if (!item.item_code) {
				...
				if (
					["Material Receipt", "Material Transfer", "Material Issue"].includes(
						doc.purpose
					)
				) {
				...
				return {
					query: "hmmerp.controllers.queries.get_batch_no",
					filters: filters,
				};
			}
		});
	...
	setup_quality_inspection: function (frm) {
		...
		if (!frm.is_new() && frm.doc.docstatus === 0) {
			frm.add_custom_button(
				...
				() => {
					let transaction_controller = new hmmerp.TransactionController({
						frm: frm,
					});
		...
		frm.set_query("quality_inspection", "items", function (doc, cdt, cdn) {
			...
			return {
				query: "hmmerp.stock.doctype.quality_inspection.quality_inspection.quality_inspection_query",
				filters: {
	...
	refresh: function (frm) {
		...
		if (frm.doc.items) {
			...
			if (frm.doc.docstatus == 0 && has_alternative) {
				frm.add_custom_button(__("Alternate Item"), () => {
					hmmerp.utils.select_alternate_items({
						frm: frm,
						child_docname: "items",
			...
			if (
				...
				frm.add_custom_button(__("End Transit"), function () {
					frappe.model.open_mapped_doc({
						method: "hmmerp.stock.doctype.stock_entry.stock_entry.make_stock_in_entry",
						frm: frm,
					});
				});
			}
		...
		if (frm.doc.docstatus === 0) {
			frm.add_custom_button(
				...
				function () {
					hmmerp.utils.map_current_doc({
						method: "hmmerp.accounts.doctype.purchase_invoice.purchase_invoice.make_stock_entry",
			...
			frm.add_custom_button(
				...
				function () {
					...
					const d = hmmerp.utils.map_current_doc({
						method: "hmmerp.stock.doctype.material_request.material_request.make_stock_entry",
		...
		if (frm.doc.docstatus === 0 && frm.doc.purpose == "Material Issue") {
			frm.add_custom_button(
				...
				function () {
					frappe.call({
						method: "hmmerp.stock.doctype.stock_entry.stock_entry.get_expired_batch_items",
						freeze: true,
						callback: function (r) {
							if (!r.exc && r.message) {
								frm.set_value("items", []);
	...
	validate_purpose_consumption: function (frm) {
		frappe
			.call({
				method: "hmmerp.manufacturing.doctype.manufacturing_settings.manufacturing_settings.is_material_consumption_enabled",
			})
			.then((r) => {
				if (
					cint(r.message) == 0 &&
	...
	make_retention_stock_entry: function (frm) {
		frappe.call({
			method: "hmmerp.stock.doctype.stock_entry.stock_entry.move_sample_to_retention_warehouse",
			args: {
	...
	toggle_display_account_head: function (frm) {
		var enabled = hmmerp.is_perpetual_inventory_enabled(frm.doc.company);
	...
	set_basic_rate: function (frm, cdt, cdn) {
		...
		if (item.item_code || item.serial_no) {
			frappe.call({
				method: "hmmerp.stock.utils.get_incoming_rate",
				args: {
					args: args,
				},
				callback: function (r) {
	...
	get_warehouse_details: function (frm, cdt, cdn) {
		...
		if (!child.bom_no) {
			frappe.call({
				method: "hmmerp.stock.doctype.stock_entry.stock_entry.get_warehouse_details",
				args: {
					args: {
	...
	get_items_from_bom: function (frm) {
		...
		d.get_input("fetch").on("click", function () {
			...
			frappe.call({
				method: "hmmerp.manufacturing.doctype.bom.bom.get_bom_items",
				args: values,
				callback: function (r) {
					if (!r.message) {
						frappe.throw(__("BOM does not contain any stock item"));
					} else {
						hmmerp.utils.remove_empty_first_row(frm, "items");
	...
	source_warehouse_address: function (frm) {
		hmmerp.utils.get_address_display(
			frm,
	...
	apply_putaway_rule: function (frm) {
		if (frm.doc.apply_putaway_rule) hmmerp.apply_putaway_rule(frm, frm.doc.purpose);
	},
	...
	purchase_order: (frm) => {
		if (frm.doc.purchase_order) {
			...
			hmmerp.utils.map_current_doc({
				method: "hmmerp.stock.doctype.stock_entry.stock_entry.get_items_from_subcontract_order",
...
frappe.ui.form.on("Stock Entry Detail", {
	...
	uom(doc, cdt, cdn) {
		...
		if (d.uom && d.item_code) {
			return frappe.call({
				method: "hmmerp.stock.doctype.stock_entry.stock_entry.get_uom_details",
				args: {
					item_code: d.item_code,
					uom: d.uom,
					qty: d.qty,
				...
				callback: function (r) {
					if (r.message) {
						...
						if (
							...
							hmmerp.stock.select_batch_and_serial_no(frm, d);
						} else {
	...
	expense_account(frm, cdt, cdn) {
		hmmerp.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "expense_account");
	},

	cost_center(frm, cdt, cdn) {
		hmmerp.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "cost_center");
	},
	...
	add_serial_batch_bundle(frm, cdt, cdn) {
		var child = locals[cdt][cdn];
		hmmerp.stock.select_batch_and_serial_no(frm, child);
	},
});
...
var validate_sample_quantity = function (frm, cdt, cdn) {
	...
	if (d.sample_quantity && frm.doc.purpose == "Material Receipt") {
		frappe.call({
			method: "hmmerp.stock.doctype.stock_entry.stock_entry.validate_sample_quantity",
			args: {
				batch_no: d.batch_no,
				item_code: d.item_code,
...
hmmerp.stock.StockEntry = class StockEntry extends hmmerp.stock.StockController {
	setup() {
		var me = this;
		...
		this.frm.fields_dict.items.grid.get_field("item_code").get_query = function () {
			return hmmerp.queries.item({ is_stock_item: 1 });
		};
		...
		this.frm.fields_dict.items.grid.get_field("expense_account").get_query = function () {
			if (hmmerp.is_perpetual_inventory_enabled(me.frm.doc.company)) {
				return {
					filters: {
						company: me.frm.doc.company,
						is_group: 0,
		...
		this.frm.set_query("supplier_address", hmmerp.queries.address_query);
	}
	...
	refresh() {
		var me = this;
		hmmerp.toggle_naming_series();
		this.toggle_related_fields(this.frm.doc);
		this.toggle_enable_bom();
		this.show_stock_ledger();
		this.set_fields_onload_for_line_item();
		hmmerp.utils.view_serial_batch_nos(this.frm);
		if (
			this.frm.doc.docstatus === 1 &&
			hmmerp.is_perpetual_inventory_enabled(this.frm.doc.company)
		) {
			this.show_general_ledger();
		}
		hmmerp.hide_company(this.frm);
		hmmerp.utils.add_item(this.frm);
		hmmerp.accounts.ledger_preview.show_accounting_ledger_preview(this.frm);
		hmmerp.accounts.ledger_preview.show_stock_ledger_preview(this.frm);
	}
	...
	scan_barcode() {
		frappe.flags.dialog_set = false;
		const barcode_scanner = new hmmerp.utils.BarcodeScanner({ frm: this.frm });
	...
	company() {
		if (this.frm.doc.company) {
			...
			hmmerp.accounts.dimensions.update_dimension(this.frm, this.frm.doctype);
	...
	set_default_account(company_fieldname, fieldname) {
		...
		return this.frm.call({
			method: "hmmerp.accounts.utils.get_company_default",
			args: {
			...
			return this.frm.call({
				...
				callback: function (r) {
					...
					if (me.frm.doc.bom_no) {
						...
						hmmerp.accounts.dimensions.update_dimension(me.frm, me.frm.doctype);
					}
				},
			});
		}
	...
	work_order() {
		...
		return frappe.call({
			method: "hmmerp.stock.doctype.stock_entry.stock_entry.get_work_order_details",
			args: {
			...
			callback: function (r) {
				if (!r.exc) {
					...
					if (
						me.frm.doc.purpose == "Material Transfer for Manufacture" &&
	...
	items_on_form_rendered(doc, grid_row) {
		hmmerp.setup_serial_or_batch_no();
	}
	...
	toggle_related_fields(doc) {
		...
		if (hmmerp.stock.is_subcontracting_or_return_transfer(doc)) {
			doc.customer =
				doc.customer_name =
	...
	supplier(doc) {
		hmmerp.utils.get_party_details(this.frm, null, null, null);
	}
};

hmmerp.stock.select_batch_and_serial_no = (frm, item) => {
	let path = "assets/hmmerp/js/utils/serial_no_batch_selector.js";
	...
	frappe.db.get_value("Item", item.item_code, ["has_batch_no", "has_serial_no"]).then((r) => {
		if (r.message && (r.message.has_batch_no || r.message.has_serial_no)) {
			...
			new hmmerp.SerialBatchPackageSelector(frm, item, (r) => {
				if (r) {
...
hmmerp.stock.is_subcontracting_or_return_transfer = (doc) => {
	return (
		doc.purpose == "Send to Subcontractor" ||
		(doc.purpose == "Material Transfer" && doc.is_return)
	);
};
...
function attach_bom_items(bom_no) {
	...
	frappe.db.get_doc("BOM", bom_no).then((bom) => {
		...
		hmmerp.stock.bom = { name, items: {} };
		items.forEach((item) => {
			hmmerp.stock.bom.items[item.item_code] = item;
		});
	});
}
...
function check_should_not_attach_bom_items(bom_no) {
	return (
		bom_no === undefined || (hmmerp.stock.bom && hmmerp.stock.bom.name === bom_no)
	);
}

extend_cscript(cur_frm.cscript, new hmmerp.stock.StockEntry({ frm: cur_frm }));
