...
frappe.ui.form.on("Stock Reconciliation", {
	...
	set_warehouse: function (frm) {
		let transaction_controller = new hmmerp.TransactionController({ frm: frm });
	...
	get_items: function (frm) {
		...
		frappe.prompt(
			...
			function (data) {
				frappe.call({
					method: "hmmerp.stock.doctype.stock_reconciliation.stock_reconciliation.get_items",
					args: {
	...
	set_valuation_rate_and_qty: function (frm, cdt, cdn) {
		...
		if (d.item_code && d.warehouse) {
			frappe.call({
				method: "hmmerp.stock.doctype.stock_reconciliation.stock_reconciliation.get_stock_balance_for",
				args: {
	...
	toggle_display_account_head: function (frm) {
		frm.toggle_display(
			...
			hmmerp.is_perpetual_inventory_enabled(frm.doc.company)
		);
	},
	...
	set_expense_account: function (frm) {
		if (frm.doc.company && hmmerp.is_perpetual_inventory_enabled(frm.doc.company)) {
			return frm.call({
				method: "hmmerp.stock.doctype.stock_reconciliation.stock_reconciliation.get_difference_account",
				args: {
...
frappe.ui.form.on("Stock Reconciliation Item", {
	...
	add_serial_batch_bundle(frm, cdt, cdn) {
		hmmerp.utils.pick_serial_and_batch_bundle(frm, cdt, cdn, "Inward");
	},
});

hmmerp.stock.StockReconciliation = class StockReconciliation extends hmmerp.stock.StockController {
	setup() {
		var me = this;

		this.setup_posting_date_time_check();

		if (me.frm.doc.company && hmmerp.is_perpetual_inventory_enabled(me.frm.doc.company)) {
	...
	refresh() {
		if (this.frm.doc.docstatus > 0) {
			...
			if (hmmerp.is_perpetual_inventory_enabled(this.frm.doc.company)) {
...
cur_frm.cscript = new hmmerp.stock.StockReconciliation({ frm: cur_frm });
