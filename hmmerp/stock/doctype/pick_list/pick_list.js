...
frappe.ui.form.on("Pick List", {
	...
	setup: (frm) => {
		...
		frm.set_query("work_order", () => {
			return {
				query: "hmmerp.stock.doctype.pick_list.pick_list.get_pending_work_orders",
				filters: {
					company: frm.doc.company,
				},
			};
				...
				frappe.prompt(
					...
					(data) => {
						...
						hmmerp.utils.map_current_doc({
							method: "hmmerp.manufacturing.doctype.work_order.work_order.create_pick_list",
							target: frm,
	...
	create_delivery_note: (frm) => {
		frappe.model.open_mapped_doc({
			method: "hmmerp.stock.doctype.pick_list.pick_list.create_delivery_note",
			frm: frm,
		});
	},
	create_stock_entry: (frm) => {
		frappe
			.xcall("hmmerp.stock.doctype.pick_list.pick_list.create_stock_entry", {
				pick_list: frm.doc,
			})
			.then((stock_entry) => {
	...
	scan_barcode: (frm) => {
		...
		const barcode_scanner = new hmmerp.utils.BarcodeScanner(opts);
...
function get_item_details(item_code, uom = null) {
	if (item_code) {
		return frappe.xcall("hmmerp.stock.doctype.pick_list.pick_list.get_item_details", {
			item_code,
			uom,
		});
	}
