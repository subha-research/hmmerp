...
frappe.ui.form.on("Delivery Trip", {
	...
	driver: function (frm) {
		if (frm.doc.driver) {
			frappe.call({
				method: "hmmerp.stock.doctype.delivery_trip.delivery_trip.get_driver_email",
				args: {
		...
		frappe.db.get_value("Delivery Settings", { name: "Delivery Settings" }, "dispatch_template", (r) => {
			if (!r.dispatch_template) {
				...
				frappe.confirm(__("Do you want to notify all the customers by email?"), function () {
					frappe.call({
						method: "hmmerp.stock.doctype.delivery_trip.delivery_trip.notify_customers",
						args: {
...
frappe.ui.form.on("Delivery Stop", {
	customer: function (frm, cdt, cdn) {
		...
		if (row.customer) {
			frappe.call({
				method: "hmmerp.stock.doctype.delivery_trip.delivery_trip.get_contact_and_address",
	...
	contact: function (frm, cdt, cdn) {
		...
		if (row.contact) {
			frappe.call({
				method: "hmmerp.stock.doctype.delivery_trip.delivery_trip.get_contact_display",
