...
import hmmerp
from hmmerp.accounts.general_ledger import validate_accounting_period
from hmmerp.accounts.utils import get_future_stock_vouchers, repost_gle_for_stock_vouchers
from hmmerp.stock.stock_ledger import (
...
def repost_gl_entries(doc):
	if not cint(hmmerp.is_perpetual_inventory_enabled(doc.company)):
		return
...
def execute_repost_item_valuation():
	...
	if name := frappe.db.get_value(
		"Scheduled Job Type",
		{"method": "hmmerp.stock.doctype.repost_item_valuation.repost_item_valuation.repost_entries"},
		"name",
	):
