...
frappe.ui.form.on("Delivery Note", {
	setup: function (frm) {
		...
		hmmerp.queries.setup_warehouse_query(frm);
		...
		frm.set_query("expense_account", "items", function (doc, cdt, cdn) {
			if (hmmerp.is_perpetual_inventory_enabled(doc.company)) {
				return {
					filters: {
	...
	print_without_amount: function (frm) {
		hmmerp.stock.delivery_note.set_print_hide(frm.doc);
	},
		...
		if (
			...
			if (internal) {
				...
				frm.add_custom_button(
					...
					function () {
						frappe.model.open_mapped_doc({
							method: "hmmerp.stock.doctype.delivery_note.delivery_note.make_inter_company_purchase_receipt",
							frm: frm,
						});
					},
					__("Create")
...
hmmerp.stock.DeliveryNoteController = class DeliveryNoteController extends (
	...
	refresh(doc, dt, dn) {
		...
		if (
			...
			this.frm.add_custom_button(
				...
				function () {
					...
					hmmerp.utils.map_current_doc({
						method: "hmmerp.stock.doctype.pick_list.pick_list.create_dn_for_pick_lists",
						...
						get_query_method: "hmmerp.stock.doctype.pick_list.pick_list.get_pick_list_query",
						size: "extra-large",
					});
				},
				__("Get Items From")
		...
		if (!doc.is_return && doc.status != "Closed") {
			...
			if (
				...
				this.frm.add_custom_button(
					...
					function () {
						frappe.model.open_mapped_doc({
							method: "hmmerp.stock.doctype.delivery_note.delivery_note.make_packing_slip",
							frm: me.frm,
						});
					},
					__("Create")
		...
		hmmerp.stock.delivery_note.set_print_hide(doc, dt, dn);
	}

	make_shipment() {
		frappe.model.open_mapped_doc({
			method: "hmmerp.stock.doctype.delivery_note.delivery_note.make_shipment",
			frm: this.frm,
		});
	}
	...
	make_installation_note() {
		frappe.model.open_mapped_doc({
			method: "hmmerp.stock.doctype.delivery_note.delivery_note.make_installation_note",
			frm: this.frm,
		});
	}
	...
	make_sales_return() {
		frappe.model.open_mapped_doc({
			method: "hmmerp.stock.doctype.delivery_note.delivery_note.make_sales_return",
			frm: this.frm,
		});
	}
	...
	make_delivery_trip() {
		frappe.model.open_mapped_doc({
			method: "hmmerp.stock.doctype.delivery_note.delivery_note.make_delivery_trip",
			frm: cur_frm,
		});
	}
	...
	update_status(status) {
		...
		frappe.call({
			method: "hmmerp.stock.doctype.delivery_note.delivery_note.update_delivery_note_status",
...
extend_cscript(cur_frm.cscript, new hmmerp.stock.DeliveryNoteController({ frm: cur_frm }));
...
frappe.ui.form.on("Delivery Note", {
	...
	unhide_account_head: function (frm) {
		...
		var aii_enabled = hmmerp.is_perpetual_inventory_enabled(frm.doc.company);
		frm.fields_dict["items"].grid.set_column_disp(["expense_account", "cost_center"], aii_enabled);
	},
});

hmmerp.stock.delivery_note.set_print_hide = function (doc, cdt, cdn) {
