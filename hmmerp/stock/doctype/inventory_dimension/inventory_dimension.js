...
frappe.ui.form.on("Inventory Dimension", {
	...
	set_query_on_fields(frm) {
		...
		frm.set_query("document_type", () => {
			return {
				query: "hmmerp.stock.doctype.inventory_dimension.inventory_dimension.get_inventory_documents",
			};
		});
	},
	...
	set_parent_fields(frm) {
		if (frm.doc.apply_to_all_doctypes) {
			...
			frappe.call({
				method: "hmmerp.stock.doctype.inventory_dimension.inventory_dimension.get_parent_fields",
				args: {
	...
	delete_dimension(frm) {
		...
		frappe.confirm(__(msg, [frm.doc.name.bold()]), () => {
			frappe.call({
				method: "hmmerp.stock.doctype.inventory_dimension.inventory_dimension.delete_dimension",
				args: {
