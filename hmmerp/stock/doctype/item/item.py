...
import hmmerp
from hmmerp.controllers.item_variant import (
...
from hmmerp.stock.doctype.item_default.item_default import ItemDefault
from hmmerp.stock.utils import get_valuation_method
...
class Item(Document):
	...
	if TYPE_CHECKING:
		...
		from hmmerp.stock.doctype.item_barcode.item_barcode import ItemBarcode
		from hmmerp.stock.doctype.item_customer_detail.item_customer_detail import ItemCustomerDetail
		from hmmerp.stock.doctype.item_default.item_default import ItemDefault
		from hmmerp.stock.doctype.item_reorder.item_reorder import ItemReorder
		from hmmerp.stock.doctype.item_supplier.item_supplier import ItemSupplier
		from hmmerp.stock.doctype.item_tax.item_tax import ItemTax
		from hmmerp.stock.doctype.item_variant_attribute.item_variant_attribute import ItemVariantAttribute
		from hmmerp.stock.doctype.uom_conversion_detail.uom_conversion_detail import UOMConversionDetail
			...
			for item_barcode in self.barcodes:
				...
				if item_barcode.barcode:
					...
					if item_barcode.barcode_type:
						barcode_type = convert_hmmerp_to_barcodenumber(
	...
	def recalculate_bin_qty(self, new_name):
		from hmmerp.stock.stock_balance import repost_stock
		...
		if self.has_variants:
			...
			if variants:
				if len(variants) <= 30:
					...
					frappe.enqueue(
						"hmmerp.stock.doctype.item.item.update_variants",
						variants=variants,
						template=self,
						now=frappe.in_test,
						timeout=600,
...
def convert_hmmerp_to_barcodenumber(hmmerp_number, barcode):
	if hmmerp_number == "EAN":
		ean_type = {
			8: "EAN8",
			13: "EAN13",
		}
	...
	if hmmerp_number in convert:
		return convert[hmmerp_number]
...
def get_asset_naming_series():
	from hmmerp.assets.doctype.asset.asset import get_asset_naming_series
