...
frappe.ui.form.on("Item", {
	...
	refresh: function (frm) {
		...
		if (frm.doc.has_variants) {
			...
			if (frm.doc.variant_based_on === "Item Attribute") {
				frm.add_custom_button(
					__("Single Variant"),
					function () {
						hmmerp.item.show_single_variant_dialog(frm);
					},
					__("Create")
				);
				frm.add_custom_button(
					...
					function () {
						hmmerp.item.show_multiple_variants_dialog(frm);
					},
					__("Create")
				);
			} else {
				frm.add_custom_button(
					__("Variant"),
					function () {
						hmmerp.item.show_modal_for_manufacturers(frm);
					},
					__("Create")
				);
			}
		...
		hmmerp.item.edit_prices_button(frm);
		hmmerp.item.toggle_attributes(frm);

		if (!frm.doc.is_fixed_asset) {
			hmmerp.item.make_dashboard(frm);
		}
	...
	validate: function (frm) {
		hmmerp.item.weight_to_validate(frm);
	},
	...
	is_fixed_asset: function (frm) {
		...
		frappe.call({
			method: "hmmerp.stock.doctype.item.item.get_asset_naming_series",
			callback: function (r) {
...
$.extend(hmmerp.item, {
	...
	make_dashboard: function (frm) {
		...
		if (frm.doc.is_stock_item) {
			frappe.require("item-dashboard.bundle.js", function () {
				...
				hmmerp.item.item_dashboard = new hmmerp.stock.ItemDashboard({
					parent: section,
					item_code: frm.doc.name,
					page_length: 20,
					method: "hmmerp.stock.dashboard.item_dashboard.get_data",
					template: "item_dashboard_list",
				});
				hmmerp.item.item_dashboard.refresh();
			});
		}
	},
	...
	show_modal_for_manufacturers: function (frm) {
		...
		dialog.set_primary_action(__("Create"), function () {
			...
			frappe.call({
				method: "hmmerp.controllers.item_variant.get_variant",
				args: data,
				callback: function (r) {
	...
	show_multiple_variants_dialog: function (frm) {
		...
		function make_and_show_dialog(fields) {
			...
			me.multiple_variant_dialog.set_primary_action(__("Create Variants"), () => {
				...
				frappe.call({
					method: "hmmerp.controllers.item_variant.enqueue_multiple_variant_creation",
					args: {
						item: frm.doc.name,
			...
			frappe.call({
				...
				callback: function (r) {
					...
					if (r.message) {
						...
						d.hide();
						frappe.call({
							method: "hmmerp.controllers.item_variant.create_variant",
							args: {
								item: frm.doc.name,
								args: d.get_values(),
		...
		$.each(d.fields_dict, function (i, field) {
			...
			field.$input
				.on("input", function (e) {
					...
					frappe.call({
						method: "hmmerp.stock.doctype.item.item.get_item_attribute",
						args: {
							parent: i,
...
frappe.ui.form.on("UOM Conversion Detail", {
	uom: function (frm, cdt, cdn) {
		...
		if (row.uom) {
			frappe.call({
				method: "hmmerp.stock.doctype.item.item.get_uom_conv_factor",
				args: {
					uom: row.uom,
