...
hmmerp.stock.ItemDashboard = class ItemDashboard {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
		...
		function handle_move_add(element, action) {
			...
			if (disable_quick_entry) {
				...
				if (action === "Add") {
					...
					hmmerp.stock.move_item(item, null, warehouse, actual_qty, rate, stock_uom, function () {
						me.refresh();
					});
				} else {
					hmmerp.stock.move_item(item, warehouse, null, actual_qty, null, stock_uom, function () {
						me.refresh();
					});
				}
			}
...
hmmerp.stock.move_item = function (item, source, target, actual_qty, rate, stock_uom, callback) {
