...
from hmmerp.accounts.doctype.pricing_rule.pricing_rule import (
...
from hmmerp.stock.doctype.item.item import get_item_defaults, get_uom_conv_factor
from hmmerp.stock.doctype.item_manufacturer.item_manufacturer import get_item_manufacturer_part_no
from hmmerp.stock.doctype.price_list.price_list import get_price_list_details
...
@frappe.whitelist()
@hmmerp.normalize_ctx_input(ItemDetailsCtx)
...
def update_stock(ctx, out, doc=None):
	from hmmerp.stock.doctype.batch.batch import get_available_batches
	from hmmerp.stock.doctype.serial_no.serial_no import get_serial_nos_for_outward
...
def validate_item_details(ctx: ItemDetailsCtx, item):
	...
	from hmmerp.stock.doctype.item.item import validate_end_of_life
...
def get_basic_details(ctx: ItemDetailsCtx, item, overwrite_warehouse=True) -> ItemDetails:
	...
	if item.is_fixed_asset:
		from hmmerp.assets.doctype.asset.asset import get_asset_account, is_cwip_accounting_enabled
	...
	if ctx.doctype in purchase_doctypes and not frappe.db.get_single_value(
		...
		from hmmerp.buying.doctype.purchase_order.purchase_order import item_last_purchase_rate
...
from hmmerp.deprecation_dumpster import get_item_warehouse
...
def get_default_cost_center(ctx: ItemDetailsCtx, item=None, item_group=None, brand=None, company=None):
	...
	if not cost_center and (item and item_group and brand):
		...
		for method in ["get_item_defaults", "get_item_group_defaults", "get_brand_defaults"]:
			path = f"hmmerp.stock.get_item_details.{method}"
...
def get_price_list_rate(ctx: ItemDetailsCtx, item_doc, out: ItemDetails = None):
	...
	if meta.get_field("currency") or ctx.get("currency"):
		...
		if not ctx.is_internal_supplier and not out.price_list_rate and ctx.transaction_type == "buying":
			from hmmerp.stock.doctype.item.item import get_last_purchase_details
...
def validate_conversion_rate(ctx: ItemDetailsCtx, meta):
	from hmmerp.controllers.accounts_controller import validate_conversion_rate
...
from hmmerp.deprecation_dumpster import get_pos_profile_item_details
...
def get_bin_details(item_code, warehouse, company=None, include_child_warehouses=False):
	...
	if warehouse:
		...
		from hmmerp.stock.doctype.warehouse.warehouse import get_child_warehouses
...
def get_batch_qty(batch_no, warehouse, item_code):
	from hmmerp.stock.doctype.batch import batch
