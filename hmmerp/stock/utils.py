...
from hmmerp.stock.serial_batch_bundle import BatchNoValuation, SerialNoValuation
from hmmerp.stock.valuation import FIFOValuation, LIFOValuation
...
def get_incoming_rate(args, raise_error_if_no_rate=True):
	...
	from hmmerp.stock.stock_ledger import get_previous_sle, get_valuation_rate
	...
	if in_rate is None:
		...
		in_rate = get_valuation_rate(
			...
			currency=hmmerp.get_company_currency(args.get("company")),
	...
	if reposting_pending and throw_error:
		...
		frappe.msgprint(
			...
			primary_action={
				...
				"client_action": "hmmerp.route_to_pending_reposts",
				"args": filters,
			},
		)
...
def scan_barcode(search_value: str) -> BarcodeScanResult:
	def set_cache(data: BarcodeScanResult):
		frappe.cache().set_value(f"hmmerp:barcode_scan:{search_value}", data, expires_in_sec=120)

	def get_cache() -> BarcodeScanResult | None:
		if data := frappe.cache().get_value(f"hmmerp:barcode_scan:{search_value}"):
			return data
