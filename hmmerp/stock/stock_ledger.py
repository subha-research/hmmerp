...
import hmmerp
from hmmerp.stock.doctype.bin.bin import update_qty as update_bin_qty
...
from hmmerp.stock.utils import (
...
from hmmerp.stock.valuation import FIFOValuation, LIFOValuation, round_off_if_near_zero
...
def make_sl_entries(sl_entries, allow_negative_stock=False, via_landed_cost_voucher=False):
	...
	args:
	        ...
	        such cases certain validations need to be ignored (like negative
	                        stock)
	"""
	from hmmerp.controllers.stock_controller import future_sle_exists
...
class update_entries_after:
	...
	def update_serial_no_status(self, sle):
		from hmmerp.stock.serial_batch_bundle import get_serial_nos
	...
	def update_batched_values(self, sle):
		from hmmerp.stock.serial_batch_bundle import BatchNoValuation
	...
	def get_fallback_rate(self, sle) -> float:
		...
		return get_valuation_rate(
			...
			currency=hmmerp.get_company_currency(sle.company),
			company=sle.company,
		)
...
def get_valuation_rate(
	...
	if (
		...
		and cint(hmmerp.is_perpetual_inventory_enabled(company))
	):
