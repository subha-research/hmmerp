...
frappe.provide("svasamm_erp.integrations");
	...
	refresh: function (frm) {
		if (frm.doc.enabled) {
			frm.add_custom_button(__("Link a new bank account"), () => {
				new svasamm_erp.integrations.plaidLink(frm);
			});
			...
			frm.add_custom_button(__("Sync Now"), () => {
				frappe.call({
					method: "svasamm_erp.svasamm_erp_integrations.doctype.plaid_settings.plaid_settings.enqueue_synchronization",
					freeze: true,
					callback: () => {
...
svasamm_erp.integrations.plaidLink = class plaidLink {
	constructor(parent) {
		this.frm = parent;
	...
	plaid_success(token, response) {
		...
		frappe.prompt(
			...
			(data) => {
				...
				frappe
					.xcall(
						"svasamm_erp.svasamm_erp_integrations.doctype.plaid_settings.plaid_settings.add_institution",
						{
							token: token,
							response: response,
						}
					...
					.then((result) => {
						frappe.xcall(
							"svasamm_erp.svasamm_erp_integrations.doctype.plaid_settings.plaid_settings.add_bank_accounts",
							{
								response: response,
								bank: result,
								company: me.company,
