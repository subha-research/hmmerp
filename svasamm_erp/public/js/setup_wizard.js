frappe.provide("svasamm_erp.setup");
...
frappe.setup.on("before_load", function () {
	if (
		...
		frappe.boot.setup_wizard_completed_apps.includes("svasamm_erp")
	) {
		return;
	}

	svasamm_erp.setup.slides_settings.map(frappe.setup.add_slide);
});

svasamm_erp.setup.slides_settings = [
	{
		// Organization
		name: "organization",
		...
		set_fy_dates: function (slide) {
			...
			if (country) {
				let fy = svasamm_erp.setup.fiscal_years[country];
		...
		charts_modal: function (slide, chart_template) {
			...
			let coa_tree = new frappe.ui.Tree({
				...
				label: parent,
				expandable: true,
				method: "svasamm_erp.accounts.utils.get_coa",
				args: {
...
svasamm_erp.setup.fiscal_years = {
