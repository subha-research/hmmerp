...
frappe.provide("svasamm_erp");
frappe.provide("svasamm_erp.utils");
frappe.provide("svasamm_erp.stock.utils");
...
$.extend(svasamm_erp, {
	...
	hide_company: function (frm) {
		if (frm?.fields_dict.company) {
			...
			} else if (svasamm_erp.last_selected_company) {
				if (!frm.doc.company) frm.set_value("company", svasamm_erp.last_selected_company);
			}
		}
	},
...
$.extend(svasamm_erp.utils, {
	set_party_dashboard_indicators: function (frm) {
		if (frm.doc.__onload && frm.doc.__onload.dashboard_info) {
			var company_wise_info = frm.doc.__onload.dashboard_info;
			if (company_wise_info.length > 1) {
				company_wise_info.forEach(function (info) {
					svasamm_erp.utils.add_indicator_for_multicompany(frm, info);
				});
	...
	get_terms: function (tc_name, doc, callback) {
		if (tc_name) {
			return frappe.call({
				method: "svasamm_erp.setup.doctype.terms_and_conditions.terms_and_conditions.get_terms_and_conditions",
				args: {
	...
	make_bank_account: function (doctype, docname) {
		frappe.call({
			method: "svasamm_erp.accounts.doctype.bank_account.bank_account.make_bank_account",
			args: {
				doctype: doctype,
				docname: docname,
			},
	...
	add_dimensions: function (report_name, index) {
		...
		frappe.call({
			method: "svasamm_erp.accounts.doctype.accounting_dimension.accounting_dimension.get_dimensions",
			callback: function (r) {
	...
	add_inventory_dimensions: function (report_name, index) {
		...
		frappe.call({
			method: "svasamm_erp.stock.doctype.inventory_dimension.inventory_dimension.get_inventory_dimensions",
			callback: function (r) {
	...
	make_pricing_rule: function (doctype, docname) {
		frappe.call({
			method: "svasamm_erp.accounts.doctype.pricing_rule.pricing_rule.make_pricing_rule",
			args: {
				doctype: doctype,
				docname: docname,
			},
	...
	get_fiscal_year: function (date, with_dates = false, raise_on_missing = true) {
		...
		if (
			...
			frappe.call({
				method: "svasamm_erp.accounts.utils.get_fiscal_year",
...
svasamm_erp.utils.select_alternate_items = function (opts) {
	const frm = opts.frm;
			...
			{ fieldtype: "Section Break", label: __("Items") },
			{
				...
				fields: [
					...
					},
					{
						...
						get_query: (e) => {
							return {
								query: "svasamm_erp.stock.doctype.item_alternative.item_alternative.get_alternative_items",
								filters: {
...
svasamm_erp.utils.update_child_items = function (opts) {
	const frm = opts.frm;
	...
	let dialog = new frappe.ui.Dialog({
		...
		update_items: function () {
			...
			frappe.call({
				method: "svasamm_erp.controllers.accounts_controller.update_child_qty_rate",
				freeze: true,
				args: {
...
svasamm_erp.utils.map_current_doc = function (opts) {
	function _map() {
	...
	$.each(frappe.boot.service_level_agreement_doctypes, function (_i, d) {
		frappe.ui.form.on(d, {
			onload: function (frm) {
				...
				frappe.call({
					method: "svasamm_erp.support.doctype.service_level_agreement.service_level_agreement.get_service_level_agreement_filters",
					args: {
						doctype: frm.doc.doctype,
...
$.extend(svasamm_erp.stock.utils, {
	set_item_details_using_barcode(frm, child_row, callback) {
		const barcode_scanner = new svasamm_erp.utils.BarcodeScanner({ frm: frm });
