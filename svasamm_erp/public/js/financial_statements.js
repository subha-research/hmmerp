frappe.provide("svasamm_erp.financial_statements");

svasamm_erp.financial_statements = {
	filters: get_filters(),
	baseData: null,
	formatter: function (value, row, column, data, default_formatter, filter) {
		...
		if (data && column.fieldname == this.name_field) {
			...
			if (data.account || data.accounts) {
				column.link_onclick =
					"svasamm_erp.financial_statements.open_general_ledger(" + JSON.stringify(data) + ")";
			}
			column.is_tree = true;
		}
	...
	onload: function (report) {
		...
		svasamm_erp.financial_statements.filters = get_filters();

		let fiscal_year = svasamm_erp.utils.get_fiscal_year(frappe.datetime.get_today());
...
function get_filters() {
	...
	let fiscal_year = svasamm_erp.utils.get_fiscal_year(frappe.datetime.get_today(), false, false);
	if (fiscal_year) {
