...
svasamm_erp.TransactionController = class TransactionController extends svasamm_erp.taxes_and_totals {
	setup() {
		super.setup();
		let me = this;
		...
		frappe.ui.form.on(this.frm.doctype + " Item", "rate", function(frm, cdt, cdn) {
			...
			if (item.item_code && item.rate) {
				frappe.call({
					method: "svasamm_erp.stock.get_item_details.get_item_tax_template",
					args: {
						ctx: {
		...
		frappe.ui.form.on(this.frm.doctype + " Item", {
			items_add: function(frm, cdt, cdn) {
				...
				svasamm_erp.accounts.dimensions.copy_dimension_from_first_row(frm, cdt, cdn, 'items');
			}
		});
		...
		if (this.frm.fields_dict["items"].grid.get_field('blanket_order')) {
			this.frm.set_query("blanket_order", "items", function(doc, cdt, cdn) {
				...
				return {
					query: "svasamm_erp.controllers.queries.get_blanket_orders",
					filters: {
						"company": doc.company,
	...
	toggle_enable_for_stock_uom(field) {
		frappe.call({
			method: 'svasamm_erp.stock.doctype.stock_settings.stock_settings.get_enable_stock_uom_editing',
			callback: (r) => {
				if (r.message) {
	...
	make_payment_request() {
		...
		frappe.call({
			method:"svasamm_erp.accounts.doctype.payment_request.payment_request.make_payment_request",
			args: {
	...
	refresh() {
		...
		svasamm_erp.hide_company(this.frm);
		this.set_dynamic_labels();
		this.setup_sms();
		this.setup_quality_inspection();
		this.validate_has_items();
		svasamm_erp.utils.view_serial_batch_nos(this.frm);
	...
	scan_barcode() {
		...
		const barcode_scanner = new svasamm_erp.utils.BarcodeScanner({frm:this.frm});
	...
	barcode(doc, cdt, cdn)  {
		let row = locals[cdt][cdn];
		if (row.barcode) {
			svasamm_erp.stock.utils.set_item_details_using_barcode(this.frm, row, (r) => {
	...
	apply_default_taxes() {
		...
		if (taxes_and_charges_field) {
			return frappe.call({
				method: "svasamm_erp.controllers.accounts_controller.get_default_taxes_and_charges",
				args: {
	...
	send_sms() {
		var sms_man = new svasamm_erp.SMSManager(this.frm.doc);
	}
							...
							frappe.run_serially([
								...
								() => {
									if(show_batch_dialog && !frappe.flags.hide_serial_batch_dialog && !frappe.flags.dialog_set) {
										...
										svasamm_erp.show_serial_batch_selector(me.frm, d, (item) => {
	...
	company() {
		...
		if (frappe.meta.get_docfield(this.frm.doctype, "shipping_address") &&
			['Purchase Order', 'Purchase Receipt', 'Purchase Invoice'].includes(this.frm.doctype)) {
				...
				if (!is_drop_ship) {
					svasamm_erp.utils.get_shipping_address(this.frm, function() {
		...
		if(this.frm.doc.company) {
			svasamm_erp.last_selected_company = this.frm.doc.company;
		}
	}
	...
	posting_date() {
		...
		if (this.frm.doc.posting_date) {
			...
			if ((this.frm.doc.doctype == "Sales Invoice" && this.frm.doc.customer) ||
				...
				return frappe.call({
					method: "svasamm_erp.accounts.party.get_due_date",
					args: {
	...
	get_company_currency() {
		return svasamm_erp.get_currency(this.frm.doc.company);
	}

	contact_person() {
		svasamm_erp.utils.get_contact_details(this.frm);
	}
	...
	conversion_rate() {
		...
		this.frm.set_df_property("conversion_rate", "read_only", svasamm_erp.stale_rate_allowed() ? 0 : 1);
	}
	...
	uom(doc, cdt, cdn) {
		...
		if(item.item_code && item.uom) {
			return this.frm.call({
				method: "svasamm_erp.stock.get_item_details.get_conversion_factor",
				args: {
					item_code: item.item_code,
					uom: item.uom
				},
	...
	service_start_date(frm, cdt, cdn) {
		...
		if(child.service_start_date) {
			frappe.call({
				"method": "svasamm_erp.stock.get_item_details.calculate_service_end_date",
				args: {ctx: child},
				callback: function(r) {
	...
	batch_no(frm, cdt, cdn) {
		...
		if (row.batch_no) {
			...
			frappe.call({
				method: "svasamm_erp.stock.get_item_details.get_batch_based_item_price",
				args: {
					pctx: params,
					item_code: row.item_code,
				},
	...
	ignore_pricing_rule() {
		if(this.frm.doc.ignore_pricing_rule) {
			...
			return this.frm.call({
				method: "svasamm_erp.accounts.doctype.pricing_rule.pricing_rule.remove_pricing_rules",
	...
	remove_pricing_rule_for_item(item) {
		...
		if (item.pricing_rules){
			let me = this;
			return this.frm.call({
				method: "svasamm_erp.accounts.doctype.pricing_rule.pricing_rule.remove_pricing_rule_for_item",
				args: {
	...
	apply_pricing_rule(item, calculate_taxes_and_totals) {
		...
		return this.frm.call({
			method: "svasamm_erp.accounts.doctype.pricing_rule.pricing_rule.apply_pricing_rule",
	...
	apply_price_list(item, reset_plc_conversion) {
		...
		// `svasamm_erp.stock.get_item_details.apply_price_list` is sensitive to its value
		...
		return this.frm.call({
			method: "svasamm_erp.stock.get_item_details.apply_price_list",
	...
	get_terms() {
		...
		svasamm_erp.utils.get_terms(this.frm.doc.tc_name, this.frm.doc, function(r) {
			if(!r.exc) {
	...
	tax_category() {
		...
		frappe.run_serially([
			...
			() => svasamm_erp.utils.set_taxes(this.frm, "tax_category"),
		]);
	}
	...
	update_item_tax_map() {
		...
		if (item_codes.length) {
			return this.frm.call({
				method: "svasamm_erp.stock.get_item_details.get_item_tax_info",
				args: {
					doc: me.frm.doc,
	...
	item_tax_template(doc, cdt, cdn) {
		...
		if(item.item_tax_template) {
			return this.frm.call({
				method: "svasamm_erp.stock.get_item_details.get_item_tax_map",
				args: {
					doc: me.frm.doc,
	...
	setup_item_selector() {
		...
		// 	this.item_selector = new svasamm_erp.ItemSelector({frm: this.frm});
		// }
	}
	...
	make_quality_inspection() {
		...
		const dialog = new frappe.ui.Dialog({
			...
			primary_action: function () {
				...
				frappe.call({
					method: "svasamm_erp.controllers.stock_controller.make_quality_inspections",
					args: {
						doctype: me.frm.doc.doctype,
		...
		frappe.call({
			method: "svasamm_erp.controllers.stock_controller.check_item_quality_inspection",
			args: {
	...
	get_method_for_payment() {
		let method = "svasamm_erp.accounts.doctype.payment_entry.payment_entry.get_payment_entry";
		if(this.frm.doc.__onload && this.frm.doc.__onload.make_payment_via_journal_entry){
			if(['Sales Invoice', 'Purchase Invoice'].includes( this.frm.doc.doctype)){
				method = "svasamm_erp.accounts.doctype.journal_entry.journal_entry.get_payment_entry_against_invoice";
			}else {
				method= "svasamm_erp.accounts.doctype.journal_entry.journal_entry.get_payment_entry_against_order";
			}
		}
	...
	set_query_for_item_tax_template(doc, cdt, cdn) {
		...
		if(!item.item_code) {
			...
			return {
				query: "svasamm_erp.controllers.queries.get_tax_template",
				filters: filters
			}
		}
	}
	...
	payment_terms_template() {
		...
		if(doc.payment_terms_template && doc.doctype !== 'Delivery Note' && !doc.is_return) {
			...
			frappe.call({
				method: "svasamm_erp.controllers.accounts_controller.get_payment_terms",
				args: {
	...
	payment_term(doc, cdt, cdn) {
		...
		if(row.payment_term) {
			frappe.call({
				method: "svasamm_erp.controllers.accounts_controller.get_payment_term_details",
				args: {
	...
	blanket_order(doc, cdt, cdn) {
		...
		if (item.blanket_order && (item.parenttype=="Sales Order" || item.parenttype=="Purchase Order")) {
			frappe.call({
				method: "svasamm_erp.stock.get_item_details.get_blanket_order_details",
				args: {
					ctx:{
						item_code: item.item_code,
...
svasamm_erp.show_serial_batch_selector = function (frm, item_row, callback, on_close, show_dialog) {
	...
	new svasamm_erp.SerialBatchPackageSelector(frm, item_row, (r) => {
		if (r) {
			let update_values = {
...
svasamm_erp.apply_putaway_rule = (frm, purpose=null) => {
	if (!frm.doc.company) {
	...
	frappe.call({
		method: "svasamm_erp.stock.doctype.putaway_rule.putaway_rule.apply_putaway_rule",
		args: {
			doctype: frm.doctype,
			items: frm.doc.items,
			company: frm.doc.company,
