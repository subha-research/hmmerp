...
svasamm_erp.accounts.dimensions = {
		...
		if (frappe.meta.has_field(doctype, dimension)) {
			frappe.model.with_doctype(doctype, () => {
				...
				frm.set_query(dimension, parentfield, function (doc, cdt, cdn) {
					...
					return svasamm_erp.queries.get_filtered_dimensions(row, fields, dimension, doc.company);
				});
			});
		}
	},
	...
	setup_account_filters(frm, dimension, fields) {
		frm.set_query(dimension, function (doc) {
			return svasamm_erp.queries.get_filtered_dimensions(doc, fields, dimension, doc.company);
		});
	},
