svasamm_erp.SerialBatchPackageSelector = class SerialNoBatchBundleUpdate {
	...
	set_serial_nos_from_range() {
		...
		const serial_nos = svasamm_erp.stock.utils.get_serial_range(serial_no_range, "::");
	...
	create_serial_nos() {
		...
		frappe.call({
			method: "svasamm_erp.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.create_serial_nos",
			args: {
				item_code: this.item.item_code,
	...
	download_csv_file() {
		...
		const method = `/api/method/svasamm_erp.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.download_blank_csv_template?content=${encodeURIComponent(
	...
	upload_csv_file() {
		...
		frappe.call({
			method: "svasamm_erp.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.upload_csv_file",
			args: {
				item_code: this.item.item_code,
				file_path: file_path,
			},
	...
	get_auto_data() {
		...
		if (qty) {
			frappe.call({
				method: "svasamm_erp.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.get_auto_data",
				args: {
					item_code: this.item.item_code,
					warehouse: warehouse,
	...
	scan_barcode_data() {
		...
		if (scan_serial_no || scan_batch_no) {
			frappe.call({
				method: "svasamm_erp.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.is_serial_batch_no_exists",
				args: {
					item_code: this.item.item_code,
	...
	update_serial_batch_no() {
		...
		if (scan_serial_no) {
			...
			if (!this.item.has_batch_no) {
				...
				frappe.call({
					method: "svasamm_erp.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.get_batch_no_from_serial_no",
					args: {
						serial_no: scan_serial_no,
					},
					callback: (r) => {
	...
	update_bundle_entries() {
		...
		frappe
			.call({
				method: "svasamm_erp.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.add_serial_batch_ledgers",
				args: {
					entries: entries,
					child_row: this.item,
					doc: this.frm.doc,
	...
	render_data() {
		if (this.bundle || this.frm.doc.is_return) {
			frappe
				.call({
					method: "svasamm_erp.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.get_serial_batch_ledgers",
					args: {
						item_code: this.item.item_code,
						name: this.bundle,
