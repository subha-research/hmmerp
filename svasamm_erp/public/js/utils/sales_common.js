...
frappe.provide("svasamm_erp.selling");

svasamm_erp.sales_common = {
	setup_selling_controller: function () {
		svasamm_erp.selling.SellingController = class SellingController extends svasamm_erp.TransactionController {
			setup() {
				super.setup();
			...
			setup_queries() {
				...
				$.each(
					...
					function (i, opts) {
						if (me.frm.fields_dict[opts[0]]) me.frm.set_query(opts[0], svasamm_erp.queries[opts[1]]);
					}
				);

				me.frm.set_query("contact_person", svasamm_erp.queries.contact_query);
				me.frm.set_query("company_contact_person", svasamm_erp.queries.company_contact_query);
				me.frm.set_query("customer_address", svasamm_erp.queries.address_query);
				me.frm.set_query("shipping_address_name", svasamm_erp.queries.address_query);
				me.frm.set_query("dispatch_address_name", svasamm_erp.queries.dispatch_address_query);
				me.frm.set_query("company_address", svasamm_erp.queries.company_address_query);
			...
			set_default_company_address() {
				...
				if (this.frm.doc.company) {
					frappe.call({
						method: "svasamm_erp.setup.doctype.company.company.get_default_company_address",
						args: {
			...
			customer_address() {
				svasamm_erp.utils.get_address_display(this.frm, "customer_address");
				svasamm_erp.utils.set_taxes_from_address(
					this.frm,
					"customer_address",
					"customer_address",
			...
			shipping_address_name() {
				svasamm_erp.utils.get_address_display(this.frm, "shipping_address_name", "shipping_address");
				svasamm_erp.utils.set_taxes_from_address(
					this.frm,
					"shipping_address_name",
					"customer_address",
			...
			dispatch_address_name() {
				svasamm_erp.utils.get_address_display(this.frm, "dispatch_address_name", "dispatch_address");
			}
			...
			pick_serial_and_batch(doc, cdt, cdn) {
				...
				frappe.db.get_value("Item", item.item_code, ["has_batch_no", "has_serial_no"]).then((r) => {
					if (r.message && (r.message.has_batch_no || r.message.has_serial_no)) {
						...
						new svasamm_erp.SerialBatchPackageSelector(me.frm, item, (r) => {
							if (r) {
			...
			project() {
				...
				if (["Delivery Note", "Sales Invoice", "Sales Order"].includes(this.frm.doc.doctype)) {
					if (this.frm.doc.project) {
						frappe.call({
							method: "svasamm_erp.projects.doctype.project.project.get_cost_center_name",
