...
svasamm_erp.utils.get_party_details = function (frm, method, args, callback) {
	if (!method) {
	...
	if (frappe.meta.get_docfield(frm.doc.doctype, "taxes")) {
		if (
			!svasamm_erp.utils.validate_mandatory(
				frm,
...
svasamm_erp.utils.add_item = function (frm) {
	if (frm.is_new()) {
...
svasamm_erp.utils.get_address_display = function (frm, address_field, display_field, is_your_company_address) {
...
svasamm_erp.utils.set_taxes_from_address = function (
	frm,
	...
	frappe.call({
		method: "svasamm_erp.accounts.party.get_address_tax_category",
		args: {
		...
		callback: function (r) {
			if (!r.exc) {
				if (frm.doc.tax_category != r.message) {
					...
					svasamm_erp.utils.set_taxes(frm, triggered_from_field);
				}
			}
		},
	});
...
svasamm_erp.utils.set_taxes = function (frm, triggered_from_field) {
	if (frappe.meta.get_docfield(frm.doc.doctype, "taxes")) {
		if (!svasamm_erp.utils.validate_mandatory(frm, "Company", frm.doc.company, triggered_from_field)) {
			return;
		}
	...
	frappe.call({
		method: "svasamm_erp.accounts.party.set_taxes",
		args: {
			party: party,
			party_type: party_type,
...
svasamm_erp.utils.get_contact_details = function (frm) {
	if (frm.updating_party_details) return;
...
svasamm_erp.utils.validate_mandatory = function (frm, label, value, trigger_on) {
	if (!value) {
		frm.doc[trigger_on] = "";
...
svasamm_erp.utils.get_shipping_address = function (frm, callback) {
	if (frm.doc.company) {
		if (
		...
		frappe.call({
			method: "svasamm_erp.accounts.custom.address.get_shipping_address",
			args: {
				company: frm.doc.company,
