...
svasamm_erp.accounts.ledger_preview = {
		...
		if (!frm.is_new() && frm.doc.docstatus == 0) {
			frm.add_custom_button(
				...
				function () {
					frappe.call({
						type: "GET",
						method: "svasamm_erp.controllers.stock_controller.show_accounting_ledger_preview",
						args: {
		...
		if (!frm.is_new() && frm.doc.docstatus == 0) {
			frm.add_custom_button(
				...
				function () {
					frappe.call({
						type: "GET",
						method: "svasamm_erp.controllers.stock_controller.show_stock_ledger_preview",
						args: {
							company: frm.doc.company,
							doctype: frm.doc.doctype,
							docname: frm.doc.name,
