frappe.provide("svasamm_erp.bulk_transaction_processing");

$.extend(svasamm_erp.bulk_transaction_processing, {
	create: function (listview, from_doctype, to_doctype, args) {
		...
		frappe.confirm(__("Create {0} {1} ?", [count_of_rows, __(to_doctype)]), () => {
			if (doc_name.length == 0) {
				frappe
					.call({
						method: "svasamm_erp.utilities.bulk_transaction.transaction_processing",
						args: {
							data: checked_items,
