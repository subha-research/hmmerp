...
frappe.ui.form.on("BOM Update Tool", {
	...
	replace: (frm) => {
		if (frm.doc.current_bom && frm.doc.new_bom) {
			frappe.call({
				method: "svasamm_erp.manufacturing.doctype.bom_update_tool.bom_update_tool.enqueue_replace_bom",
				freeze: true,
				args: {
					boms: {
	...
	update_latest_price_in_all_boms: (frm) => {
		frappe.call({
			method: "svasamm_erp.manufacturing.doctype.bom_update_tool.bom_update_tool.enqueue_update_cost",
			freeze: true,
			callback: (result) => {
				if (result && result.message && !result.exc) {
					frm.events.confirm_job_start(frm, result.message);
