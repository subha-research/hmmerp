...
from svasamm_erp.accounts.general_ledger import get_round_off_account_and_cost_center
...
from svasamm_erp.buying.utils import update_last_purchase_rate
from svasamm_erp.controllers.print_settings import (
	set_print_templates_for_item_table,
	set_print_templates_for_taxes,
)
from svasamm_erp.controllers.sales_and_purchase_return import validate_return
from svasamm_erp.exceptions import InvalidCurrency
from svasamm_erp.setup.utils import get_exchange_rate
from svasamm_erp.stock.doctype.item.item import get_uom_conv_factor
...
class AccountsController(TransactionBase):
	...
	def company_currency(self):
		if not hasattr(self, "__company_currency"):
			self.__company_currency = svasamm_erp.get_company_currency(self.company)
	...
	def on_trash(self):
		from svasamm_erp.accounts.utils import delete_exchange_gain_loss_journal
	...
	def calculate_taxes_and_totals(self):
		from svasamm_erp.controllers.taxes_and_totals import calculate_taxes_and_totals
	...
	def validate_due_date(self):
		...
		from svasamm_erp.accounts.party import validate_due_date
			...
			for item in self.get("items"):
				if item.get("item_code"):
					...
					if hasattr(item, "cost_center") and not item.get("cost_center"):
						item.set(
							"cost_center",
							self.get("cost_center") or svasamm_erp.get_default_cost_center(self.company),
						)
	...
	def update_against_document_in_jv(self):
		...
		if lst:
			from svasamm_erp.accounts.utils import reconcile_against_document
	...
	def on_cancel(self):
		from svasamm_erp.accounts.doctype.bank_transaction.bank_transaction import (
	...
	def validate_multiple_billing(self, ref_dt, item_ref_dn, based_on):
		from svasamm_erp.controllers.status_updater import get_allowance_for
	...
	def create_advance_and_reconcile(self, party_link):
		...
		default_currency = svasamm_erp.get_company_currency(self.company)
		...
		reconcilation_entry.cost_center = self.cost_center or svasamm_erp.get_default_cost_center(self.company)

		advance_entry.account = primary_account
		advance_entry.party_type = primary_party_type
		advance_entry.party = primary_party
		advance_entry.cost_center = self.cost_center or svasamm_erp.get_default_cost_center(self.company)
