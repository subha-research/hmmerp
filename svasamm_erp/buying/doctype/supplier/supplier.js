...
frappe.ui.form.on("Supplier", {
	setup: function (frm) {
		...
		frm.set_query("supplier_primary_contact", function (doc) {
			return {
				query: "svasamm_erp.buying.doctype.supplier.supplier.get_supplier_primary_contact",
				filters: {
					supplier: doc.name,
				},
			};
	...
	refresh: function (frm) {
		...
		if (frm.doc.__islocal) {
			...
			frm.add_custom_button(
				__("Bank Account"),
				function () {
					svasamm_erp.utils.make_bank_account(frm.doc.doctype, frm.doc.name);
				},
				__("Create")
			);
			...
			frm.add_custom_button(
				__("Pricing Rule"),
				function () {
					svasamm_erp.utils.make_pricing_rule(frm.doc.doctype, frm.doc.name);
				},
				__("Create")
			);
