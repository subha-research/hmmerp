...
frappe.ui.form.on("Dunning", {
	setup: function (frm) {
		...
		frm.set_query("company_address", svasamm_erp.queries.company_address_query);
	...
	conversion_rate: function (frm) {
		if (frm.doc.currency === svasamm_erp.get_currency(frm.doc.company)) {
			frm.set_value("conversion_rate", 1.0);
		}

		// Make read only if Accounts Settings doesn't allow stale rates
		frm.set_df_property("conversion_rate", "read_only", svasamm_erp.stale_rate_allowed() ? 0 : 1);
	},
	customer_address: function (frm) {
		svasamm_erp.utils.get_address_display(frm, "customer_address");
	},
	company_address: function (frm) {
		svasamm_erp.utils.get_address_display(frm, "company_address");
	},
	...
	get_dunning_letter_text: function (frm) {
		if (frm.doc.dunning_type) {
			frappe.call({
				method: "svasamm_erp.accounts.doctype.dunning.dunning.get_dunning_letter_text",
				args: {
	...
	make_payment_entry: function (frm) {
		return frappe.call({
			method: "svasamm_erp.accounts.doctype.payment_entry.payment_entry.get_payment_entry",
			args: {
				dt: frm.doc.doctype,
				dn: frm.doc.name,
			},
