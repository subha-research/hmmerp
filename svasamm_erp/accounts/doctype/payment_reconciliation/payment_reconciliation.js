...
svasamm_erp.accounts.PaymentReconciliationController = class PaymentReconciliationController extends (
			...
			this.frm.call({
				...
				callback: (r) => {
					if (r.message) {
						this.frm
							.call({
								method: "svasamm_erp.accounts.doctype.process_payment_reconciliation.process_payment_reconciliation.is_any_doc_running",
								args: {
									for_filter: {
										company: this.frm.doc.company,
	...
	set_query_for_dimension_filters() {
		frappe.call({
			method: "svasamm_erp.accounts.doctype.payment_reconciliation.payment_reconciliation.get_queries_for_dimension_filters",
			args: {
				company: this.frm.doc.company,
			},
			callback: (r) => {
...
extend_cscript(cur_frm.cscript, new svasamm_erp.accounts.PaymentReconciliationController({ frm: cur_frm }));
