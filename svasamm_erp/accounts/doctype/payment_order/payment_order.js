...
frappe.ui.form.on("Payment Order", {
	...
	get_from_payment_entry: function (frm) {
		...
		svasamm_erp.utils.map_current_doc({
			method: "svasamm_erp.accounts.doctype.payment_entry.payment_entry.make_payment_order",
	...
	get_from_payment_request: function (frm) {
		frm.trigger("remove_row_if_empty");
		svasamm_erp.utils.map_current_doc({
			method: "svasamm_erp.accounts.doctype.payment_request.payment_request.make_payment_order",
	...
	make_payment_records: function (frm) {
			...
			fields: [
				{
					...
					get_query: function () {
						return {
							query: "svasamm_erp.accounts.doctype.payment_order.payment_order.get_supplier_query",
...

				{
					...
					get_query: function () {
						return {
							query: "svasamm_erp.accounts.doctype.payment_order.payment_order.get_mop_query",
							filters: { parent: frm.doc.name },
						};
					},
				},
		...
		dialog.set_primary_action(__("Submit"), function () {
			...
			return frappe.call({
				method: "svasamm_erp.accounts.doctype.payment_order.payment_order.make_payment_records",
				args: {
					name: frm.doc.name,
