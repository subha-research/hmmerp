...
frappe.ui.form.on("Timesheet", {
	setup: function (frm) {
		frappe.require("/assets/svasamm_erp/js/projects/timer.js");
			...
			frm.add_custom_button(__(button), function () {
				...
				$.each(frm.doc.time_logs || [], function (i, row) {
					...
					if (flag && row.activity_type && !row.from_time) {
						svasamm_erp.timesheet.timer(frm, row);
					...
					if (flag && row.from_time <= frappe.datetime.now_datetime() && !row.completed) {
						...
						svasamm_erp.timesheet.timer(frm, row, timestamp);
						flag = false;
					}
				});
				// If no activities found to start a timer, create new
				if (flag) {
					svasamm_erp.timesheet.timer(frm);
				}
	...
	make_invoice: function (frm) {
		...
		dialog.set_primary_action(__("Create Sales Invoice"), () => {
			...
			return frappe.call({
				type: "GET",
				method: "svasamm_erp.projects.doctype.timesheet.timesheet.make_sales_invoice",
				args: {
...
frappe.ui.form.on("Timesheet Detail", {
	...
	activity_type: function (frm, cdt, cdn) {
		...
		frappe.call({
			method: "svasamm_erp.projects.doctype.timesheet.timesheet.get_activity_cost",
			args: {
