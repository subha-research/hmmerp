...
svasamm_erp.landed_cost_taxes_and_charges.setup_triggers("Subcontracting Order");
...
frappe.ui.form.on("Subcontracting Order", {
	...
	set_queries: (frm) => {
		...
		frm.set_query("supplier_address", svasamm_erp.queries.address_query);

		frm.set_query("billing_address", svasamm_erp.queries.company_address_query);

		frm.set_query("shipping_address", () => {
			return svasamm_erp.queries.company_address_query(frm.doc);
		});
	},
	...
	update_subcontracting_order_status(frm, status) {
		frappe.call({
			method: "svasamm_erp.subcontracting.doctype.subcontracting_order.subcontracting_order.update_subcontracting_order_status",
			args: {
				sco: frm.doc.name,
				status: status,
			},
...
svasamm_erp.buying.SubcontractingOrderController = class SubcontractingOrderController {
	setup() {
	...
	make_subcontracting_receipt() {
		frappe.model.open_mapped_doc({
			method: "svasamm_erp.subcontracting.doctype.subcontracting_order.subcontracting_order.make_subcontracting_receipt",
			frm: cur_frm,
...
extend_cscript(cur_frm.cscript, new svasamm_erp.buying.SubcontractingOrderController({ frm: cur_frm }));
