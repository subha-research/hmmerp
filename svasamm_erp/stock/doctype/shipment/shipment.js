...
frappe.ui.form.on("Shipment", {
	...
	delivery_address_name: function (frm) {
		if (frm.doc.delivery_to_type == "Company") {
			svasamm_erp.utils.get_address_display(frm, "delivery_address_name", "delivery_address", true);
		} else {
			svasamm_erp.utils.get_address_display(frm, "delivery_address_name", "delivery_address", false);
		}
	},
	pickup_address_name: function (frm) {
		if (frm.doc.pickup_from_type == "Company") {
			svasamm_erp.utils.get_address_display(frm, "pickup_address_name", "pickup_address", true);
		} else {
			svasamm_erp.utils.get_address_display(frm, "pickup_address_name", "pickup_address", false);
		}
	},
	...
	pickup_contact_person: function (frm) {
		if (frm.doc.pickup_contact_person) {
			frappe.call({
				method: "svasamm_erp.stock.doctype.shipment.shipment.get_company_contact",
	...
	set_address_name: function (frm, ref_doctype, ref_docname, delivery_type) {
		frappe.call({
			method: "svasamm_erp.stock.doctype.shipment.shipment.get_address_name",
			args: {
	...
	set_contact_name: function (frm, ref_doctype, ref_docname, delivery_type) {
		frappe.call({
			method: "svasamm_erp.stock.doctype.shipment.shipment.get_contact_name",
			args: {
				ref_doctype: ref_doctype,
				docname: ref_docname,
			},
