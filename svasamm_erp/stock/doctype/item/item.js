...
frappe.provide("svasamm_erp.item");
...
frappe.ui.form.on("Item", {
	...
	onload: function (frm) {
		svasamm_erp.item.setup_queries(frm);
	...
	refresh: function (frm) {
		...
		if (frm.doc.has_variants) {
			...
			if (frm.doc.variant_based_on === "Item Attribute") {
				frm.add_custom_button(
					__("Single Variant"),
					function () {
						svasamm_erp.item.show_single_variant_dialog(frm);
					},
					__("Create")
				);
				frm.add_custom_button(
					...
					function () {
						svasamm_erp.item.show_multiple_variants_dialog(frm);
					},
					__("Create")
				);
			} else {
				frm.add_custom_button(
					__("Variant"),
					function () {
						svasamm_erp.item.show_modal_for_manufacturers(frm);
					},
					__("Create")
				);
			}
		...
		svasamm_erp.item.edit_prices_button(frm);
		svasamm_erp.item.toggle_attributes(frm);

		if (!frm.doc.is_fixed_asset) {
			svasamm_erp.item.make_dashboard(frm);
		}
	...
	validate: function (frm) {
		svasamm_erp.item.weight_to_validate(frm);
	},
	...
	is_fixed_asset: function (frm) {
		...
		frappe.call({
			method: "svasamm_erp.stock.doctype.item.item.get_asset_naming_series",
			callback: function (r) {
...
$.extend(svasamm_erp.item, {
	...
	make_dashboard: function (frm) {
		...
		if (frm.doc.is_stock_item) {
			frappe.require("item-dashboard.bundle.js", function () {
				...
				svasamm_erp.item.item_dashboard = new svasamm_erp.stock.ItemDashboard({
					parent: section,
					item_code: frm.doc.name,
					page_length: 20,
					method: "svasamm_erp.stock.dashboard.item_dashboard.get_data",
					template: "item_dashboard_list",
				});
				svasamm_erp.item.item_dashboard.refresh();
			});
		}
	},
	...
	show_modal_for_manufacturers: function (frm) {
		...
		dialog.set_primary_action(__("Create"), function () {
			...
			frappe.call({
				method: "svasamm_erp.controllers.item_variant.get_variant",
				args: data,
				callback: function (r) {
	...
	show_multiple_variants_dialog: function (frm) {
		...
		function make_and_show_dialog(fields) {
			...
			me.multiple_variant_dialog.set_primary_action(__("Create Variants"), () => {
				...
				frappe.call({
					method: "svasamm_erp.controllers.item_variant.enqueue_multiple_variant_creation",
					args: {
						item: frm.doc.name,
			...
			frappe.call({
				...
				callback: function (r) {
					...
					if (r.message) {
						...
						d.hide();
						frappe.call({
							method: "svasamm_erp.controllers.item_variant.create_variant",
							args: {
								item: frm.doc.name,
								args: d.get_values(),
		...
		$.each(d.fields_dict, function (i, field) {
			...
			field.$input
				.on("input", function (e) {
					...
					frappe.call({
						method: "svasamm_erp.stock.doctype.item.item.get_item_attribute",
						args: {
							parent: i,
...
frappe.ui.form.on("UOM Conversion Detail", {
	uom: function (frm, cdt, cdn) {
		...
		if (row.uom) {
			frappe.call({
				method: "svasamm_erp.stock.doctype.item.item.get_uom_conv_factor",
				args: {
					uom: row.uom,
