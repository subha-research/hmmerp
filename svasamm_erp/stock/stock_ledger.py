...
import svasamm_erp
from svasamm_erp.stock.doctype.bin.bin import update_qty as update_bin_qty
...
from svasamm_erp.stock.valuation import FIFOValuation, LIFOValuation, round_off_if_near_zero
...
class update_entries_after:
	...
	def update_batched_values(self, sle):
		from svasamm_erp.stock.serial_batch_bundle import BatchNoValuation
	...
	def get_fallback_rate(self, sle) -> float:
		...
		return get_valuation_rate(
			...
			currency=svasamm_erp.get_company_currency(sle.company),
			company=sle.company,
		)
...
def get_valuation_rate(
	...
	if (
		...
		and cint(svasamm_erp.is_perpetual_inventory_enabled(company))
	):
