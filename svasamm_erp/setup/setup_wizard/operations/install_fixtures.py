...
from svasamm_erp.accounts.doctype.account.account import RootNotEditable
from svasamm_erp.regional.address_template.setup import set_up_address_templates
...
def install(country=None):
	...
	base_path = frappe.get_app_path("svasamm_erp", "stock", "doctype")
	...
	from svasamm_erp.buying.doctype.supplier_scorecard.supplier_scorecard import make_default_records
...
def add_uom_data():
	# add UOMs
	uoms = json.loads(
		open(frappe.get_app_path("svasamm_erp", "setup", "setup_wizard", "data", "uom_data.json")).read()
	)
	for d in uoms:
	...
	uom_conversions = json.loads(
		open(
			frappe.get_app_path("svasamm_erp", "setup", "setup_wizard", "data", "uom_conversion_data.json")
		).read()
	)
