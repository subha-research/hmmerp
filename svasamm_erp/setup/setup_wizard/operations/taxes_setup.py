...
def update_regional_tax_settings(country, company):
	path = frappe.get_app_path("svasamm_erp", "regional", frappe.scrub(country))
	if os.path.exists(path.encode("utf-8")):
		try:
			module_name = f"svasamm_erp.regional.{frappe.scrub(country)}.setup.update_regional_tax_settings"
