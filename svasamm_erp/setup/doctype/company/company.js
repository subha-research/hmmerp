...
frappe.provide("svasamm_erp.company");
...
frappe.ui.form.on("Company", {
	...
	refresh: function (frm) {
		svasamm_erp.company.setup_queries(frm);
		...
		svasamm_erp.company.set_chart_of_accounts_options(frm.doc);
	},
	...
	delete_company_transactions: function (frm) {
		frappe.call({
			method: "svasamm_erp.setup.doctype.transaction_deletion_record.transaction_deletion_record.is_deletion_doc_running",
			args: {
				company: frm.doc.name,
			},
			freeze: true,
					...
					frappe.verify_password(function () {
						var d = frappe.prompt(
							...
							function (data) {
								...
								frappe.call({
									method: "svasamm_erp.setup.doctype.company.company.create_transaction_deletion_request",
									args: {
...
svasamm_erp.company.set_chart_of_accounts_options = function (doc) {
	var selected_value = doc.chart_of_accounts;
	if (doc.country) {
		return frappe.call({
			method: "svasamm_erp.accounts.doctype.account.chart_of_accounts.chart_of_accounts.get_charts_for_country",
			args: {
				country: doc.country,
				with_standard: true,
			},
...
svasamm_erp.company.setup_queries = function (frm) {
	$.each(
		[
		...
		function (i, v) {
			svasamm_erp.company.set_custom_query(frm, v);
		}
	);
...
svasamm_erp.company.set_custom_query = function (frm, v) {
	var filters = {
		company: frm.doc.name,
		is_group: 0,
	};
